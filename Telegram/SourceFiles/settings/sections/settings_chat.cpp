...
Ui::RpWidget *ColorsPalette::Button::widget() const { return const_cast<Ui::AbstractButton *>(&_widget); }
...
ColorsPalette::ColorsPalette(not_null<Ui::VerticalLayout *> container)
	...
	inner->widthValue() | rpl::on_next([=] { updateInnerGeometry(); }, inner->lifetime());
}

Ui::RpWidget *ColorsPalette::editButton() const { return _buttons.empty() ? nullptr : _buttons.back()->widget(); }
...
void ColorsPalette::show(Type type) {
	...
	const auto current =
		(settings.systemAccentColorEnabled() ? Window::Theme::SystemAccentColor() : std::optional<QColor>())
			.value_or(color.value_or(scheme->accentColor));
	...
	for (const auto &color : colors) {
		...
		if (clicks) {
			std::move(clicks) | rpl::map([=] { return _buttons[index - 1]->color(); }) |
				rpl::on_next([=](QColor color) { _selected.fire_copy(color); }, inner->lifetime());
		}
	}

	auto clicks = pushButton(std::move(colors));
	if (clicks) {
		std::move(clicks) | rpl::on_next([=] { selectCustom(scheme); }, inner->lifetime());
	}
}
...
Ui::LinkButton *BackgroundRow::chooseFromGallery() const { return _chooseFromGallery.data(); }

Ui::LinkButton *BackgroundRow::chooseFromFile() const { return _chooseFromFile.data(); }
	...
	builder.add(nullptr,
				[]
				{
					return SearchEntry{
						...
						.checkIcon = Core::App().settings().adaptiveForWide() ? SearchEntryCheckIcon::Checked
	...
	builder.add(nullptr,
				[]
				{
					return SearchEntry{
						...
						.checkIcon = Core::App().settings().largeEmoji() ? SearchEntryCheckIcon::Checked
																		 : SearchEntryCheckIcon::Unchecked,
					};
				});
	...
	builder.add(nullptr,
				[]
				{
					return SearchEntry{
						...
						.checkIcon = Core::App().settings().replaceEmoji() ? SearchEntryCheckIcon::Checked
																		   : SearchEntryCheckIcon::Unchecked,
					};
				});
	...
	builder.add(nullptr,
				[]
				{
					return SearchEntry{
						...
						.checkIcon = Core::App().settings().suggestEmoji() ? SearchEntryCheckIcon::Checked
																		   : SearchEntryCheckIcon::Unchecked,
					};
				});
	...
	builder.add(nullptr,
				[]
				{
					return SearchEntry{
						...
						.checkIcon = Core::App().settings().suggestAnimatedEmoji() ? SearchEntryCheckIcon::Checked
																				   : SearchEntryCheckIcon::Unchecked,
					};
				});
	...
	builder.add(nullptr,
				[]
				{
					return SearchEntry{
						...
						.checkIcon = Core::App().settings().suggestStickersByEmoji() ? SearchEntryCheckIcon::Checked
																					 : SearchEntryCheckIcon::Unchecked,
					};
				});
	...
	builder.add(nullptr,
				[]
				{
					return SearchEntry{
						...
						.checkIcon = Core::App().settings().loopAnimatedStickers() ? SearchEntryCheckIcon::Checked
																				   : SearchEntryCheckIcon::Unchecked,
					};
				});
	...
	builder.add(nullptr,
				[]
				{
					return SearchEntry{
						...
						.checkIcon = Core::App().settings().cornerReply() ? SearchEntryCheckIcon::Checked
																		  : SearchEntryCheckIcon::Unchecked,
					};
				});
	...
	builder.add(nullptr,
				[]
				{
					return SearchEntry{
						...
						.checkIcon = Core::App().settings().cornerReaction() ? SearchEntryCheckIcon::Checked
																			 : SearchEntryCheckIcon::Unchecked,
					};
				});
}
...
void BuildSensitiveContentSection(SectionBuilder &builder) {
	...
	builder.add(
		[controller, highlights](const WidgetContext &ctx)
		{
			...
			SetupSensitiveContent(controller, ctx.container.get(), std::move(updateOnTick), highlights);
			return SectionBuilder::WidgetToAdd{};
		},
		[]
		{
...
void BuildArchiveSection(SectionBuilder &builder) {
	...
	builder.addButton({
		...
		{
			if (controller) {
				controller->show(Box<Ui::GenericBox>(ArchiveSettingsBox, controller));
			}
		},
...
void BuildSupportSection(SectionBuilder &builder) {
	...
	builder.add(
		[controller](const WidgetContext &ctx)
		{
			...
			{
				inner->add(object_ptr<Ui::Radioenum<SwitchType>>(inner, group, value, label, st::settingsSendType),
						   st::settingsSendTypePadding);
			};
	...
	builder.add(
		[controller](const WidgetContext &ctx)
		{
			...
			const auto best = ranges::min_element(
				options, std::less<>(), [&](const Option &option) { return std::abs(option.days - days); });
			...
			const auto group = std::make_shared<Ui::RadiobuttonGroup>(best->days);
			for (const auto &option : options) {
				inner->add(object_ptr<Ui::Radiobutton>(inner, group, option.days, option.label, st::settingsSendType),
						   st::settingsSendTypePadding);
			}
			group->setChangedCallback(
				[=](int days)
...
Chat::Chat(QWidget *parent, not_null<Window::SessionController *> controller) : Section(parent, controller) {
	setupContent();
}
...
void SetupStickersEmoji(not_null<Window::SessionController *> controller,
	...
	const auto title = Ui::AddSubsectionTitle(container, tr::lng_settings_stickers_emoji());
	if (highlights) {
	...
	const auto addWithReturn = [&](const QString &label, bool checked, auto &&handle)
	{
		const auto result = inner->add(checkbox(label, checked), st::settingsCheckboxPadding);
		result->checkedChanges() | rpl::on_next(std::move(handle), inner->lifetime());
		return result;
	};
	...
	const auto largeEmoji = addWithReturn(tr::lng_settings_large_emoji(tr::now),
	...
	if (highlights) {
		highlights->push_back({u"chat/large-emoji"_q, {largeEmoji, {.radius = st::boxRadius}}});
	}

	const auto replaceEmoji = addWithReturn(tr::lng_settings_replace_emojis(tr::now),
	...
	const auto suggestEmojiCheckbox = addWithReturn(tr::lng_settings_suggest_emoji(tr::now),
	...
	if (highlights) {
		highlights->push_back({u"chat/suggest-animated-emoji"_q, {suggestAnimated, {.radius = st::boxRadius}}});
	}

	const auto suggestByEmoji = addWithReturn(tr::lng_settings_suggest_by_emoji(tr::now),
	...
	const auto loopStickers = addWithReturn(tr::lng_settings_loop_stickers(tr::now),
	...
	emojiSetsButton->addClickHandler([=] { controller->show(Box<Ui::Emoji::ManageSetsBox>(session)); });
	if (highlights) {
...
void SetupMessages(not_null<Window::SessionController *> controller,
	...
	const auto title = Ui::AddSubsectionTitle(container, tr::lng_settings_messages());
	if (highlights) {
	...
	{
		return inner->add(object_ptr<Ui::Radioenum<SendByType>>(inner, groupSend, value, text, st::settingsSendType),
						  st::settingsSendTypePadding);
	};
	const auto sendEnter = addSend(SendByType::Enter, tr::lng_settings_send_enter(tr::now));
	if (highlights) {
	...
	const auto cornerReply = inner->add(object_ptr<Ui::Checkbox>(inner,
	...
	const auto cornerReaction = inner->add(object_ptr<Ui::Checkbox>(inner,
...
void SetupChatBackground(not_null<Window::SessionController *> controller,
	...
	const auto title = Ui::AddSubsectionTitle(container, tr::lng_settings_section_background());

	const auto row = container->add(object_ptr<BackgroundRow>(container, controller), st::settingsBackgroundPadding);
	...
	tile->entity()->checkedChanges() |
		rpl::on_next([=](bool checked) { background->setTile(checked); }, tile->lifetime());
...
void SetupChatListQuickAction(not_null<Window::SessionController *> controller,
	...
	Ui::AddDivider(container);
	Ui::AddSkip(container);
	Ui::AddSubsectionTitle(container, tr::lng_settings_quick_dialog_action_title());

	using Type = Dialogs::Ui::QuickDialogAction;
	using LabelType = Dialogs::Ui::QuickDialogActionLabel;
	const auto group = std::make_shared<Ui::RadioenumGroup<Type>>(Core::App().settings().quickDialogAction());
	group->setChangedCallback(
		[=](Type value)
		{
			...
			rpl::on_next(
				[=](Type value)
				{
					...
					state->icon = Lottie::MakeIcon({
						...
						.sizeOverride =
							Size((label == LabelType::Disabled) ? int(st::dialogsQuickActionSize * kDisabledIconRatio)
																: st::dialogsQuickActionSize),
					});
					state->icon->animate([=] { widget->update(); }, 0, state->icon->framesCount() - 1);
					widget->update();
				},
				widget->lifetime());
		widget->paintRequest() |
			rpl::on_next(
				[=]
				{
					...
					const auto rightOffset = st::dialogsQuickActionRippleSize + st::dialogsQuickActionSize;
					const auto rect = QRect(widget->width() - actionWidth - st::boxRowPadding.right() - rightOffset,
											0,
											actionWidth,
											height);

					auto path = QPainterPath();
					path.addRoundedRect(QRect(-actionWidth, 0, rect::right(rect) + actionWidth, height),
										st::roundRadiusLarge,
										st::roundRadiusLarge);
					p.setClipPath(path);
					...
					p.fillRect(QRect(0, 0, rect::right(rect), st::lineWidth), st::windowBgOver);
					p.fillRect(QRect(0, rect::bottom(rect) - st::lineWidth, rect::right(rect), st::lineWidth),
							   st::windowBgOver);
					p.fillRect(rect, Dialogs::ResolveQuickActionBg(label));
					if (state->icon) {
						Dialogs::DrawQuickAction(
							p, rect, state->icon.get(), label, isDisabled ? kDisabledIconRatio : 1., isDisabled);
					}
					...
					p.drawRoundedRect(
						height, height / 2 - h * 1.5, st::dialogsQuickActionRippleSize * 0.6, h, h / 2, h / 2);
					p.drawRoundedRect(height, height / 2 + h, st::dialogsQuickActionRippleSize * 1.0, h, h / 2, h / 2);

					p.setClipping(false);
					p.resetTransform();
					p.setFont(st::settingsQuickDialogActionsTriggerFont);
					p.setPen(st::windowSubTextFg);
					p.drawText(QRect(widget->width() - st::dialogsQuickActionRippleSize - st::boxRowPadding.right(),
									 0,
									 st::dialogsQuickActionRippleSize,
									 height),
							   isDisabled ? tr::lng_settings_quick_dialog_action_swipe(tr::now)
	...
	const auto button = container->add(object_ptr<Ui::SettingsButton>(
		...
		group->value() |
			rpl::map(
				[](Type value)
				{
					return ((value == Dialogs::Ui::QuickDialogAction::Mute) ? tr::lng_settings_quick_dialog_action_mute
...

	{
		...
		button->sizeValue() |
			rpl::on_next([=, left = st.iconLeft](QSize size)
						 { icon->moveToLeft(left, (size.height() - icon->height()) / 2, size.width()); },
						 icon->lifetime());
		icon->paintRequest() |
			rpl::on_next(
				[=]
				{
					...
					((value == Dialogs::Ui::QuickDialogAction::Mute)		  ? st::menuIconMute
						 : (value == Dialogs::Ui::QuickDialogAction::Pin)	  ? st::menuIconPin
						 : (value == Dialogs::Ui::QuickDialogAction::Read)	  ? st::menuIconMarkRead
						 : (value == Dialogs::Ui::QuickDialogAction::Delete)  ? st::menuIconDelete
						 : (value == Dialogs::Ui::QuickDialogAction::Archive) ? st::menuIconArchive
					...
					const auto addRadio = [&](Type value, tr::phrase<> phrase)
					{
						box->verticalLayout()->add(
							object_ptr<Ui::Radioenum<Type>>(
								box->verticalLayout(), group, value, phrase(tr::now), st::settingsSendType),
							st::settingsSendTypePadding);
					};
...
void SetupDefaultThemes(not_null<Window::Controller *> window,
	...
	const auto systemAccentWrap =
		container->add(object_ptr<Ui::SlideWrap<Ui::Checkbox>>(
						   container,
						   object_ptr<Ui::Checkbox>(container,
		...
		ranges::views::transform(
			[&](const Scheme &scheme)
			{
				...
				rpl::duplicate(scheme.name) |
					rpl::on_next([=](const QString &themeName) { result->setText(themeName); }, result->lifetime());
	...
	const auto refreshColorizer = [=](Type type)
	{
		...
		const auto color = settings.systemAccentColorEnabled() ? Window::Theme::SystemAccentColor()
															   : settings.themesAccentColors().get(type);
		if (i != end(checks)) {
			if (AyuFeatures::MessageShot::isChoosingTheme()) {
	...
	{ systemAccentWrap->toggle(IsSystemAccentColorSupported() && (type != Type(-1)), anim::type::instant); };
	group->setChangedCallback(
		[=](Type type)
		{
			if (AyuFeatures::MessageShot::isChoosingTheme()) {
	...
	if (highlights) {
		...
		if (IsSystemAccentColorSupported()) {
			highlights->push_back(
				{u"chat/themes-system-accent"_q, {systemAccentWrap->entity(), {.radius = st::boxRadius}}});
		}
	}
...
void SetupThemeOptions(not_null<Window::SessionController *> controller,
	...
	const auto title = Ui::AddSubsectionTitle(container, tr::lng_settings_themes());
	if (highlights) {
...
void SetupCloudThemes(not_null<Window::SessionController *> controller,
	...
	list->allShown() | rpl::on_next([=](bool shown) { showAll->setVisible(!shown); }, showAll->lifetime());
...
void SetupThemeSettings(not_null<Window::SessionController *> controller,
	...
	const auto title = Ui::AddSubsectionTitle(container, tr::lng_settings_theme_settings());
	if (highlights) {
	...
	if (settings->systemDarkMode().has_value()) {
		auto label =
			...
			rpl::flatten_latest();
		const auto button = AddButtonWithLabel(container,
	...
	fontButton->setClickedCallback(
		[=]
		{
			...
			const auto theme = std::shared_ptr<Ui::ChatTheme>(Window::Theme::DefaultChatThemeOn(container->lifetime()));
			const auto generateBg = [=]
			{
				...
				auto p = QPainter(&result);
				Window::SectionWidget::PaintBackground(p, theme.get(), QSize(size, size * 3), QRect(0, 0, size, size));
				p.end();
