...
void Message::refreshRightBadge() {
	...
	if (badge->role == BadgeRole::User) {
		const auto tagWidth = badge->tag.isEmpty() ? 0 : badge->tag.maxWidth();
		badge->width = tagWidth + boostWidth;
	} else {
		const auto &padding = st::msgTagBadgePadding;
		const auto textWidth = badge->tag.maxWidth();
		const auto contentWidth = padding.left() + textWidth + padding.right();
		const auto pillHeight = padding.top() + st::msgFont->height + padding.bottom();
		...
		if (const auto badge = Get<RightBadge>()) {
			const auto badgeColor = (badge->role == BadgeRole::Creator) ? st::rankOwnerFg->c
				: (badge->role == BadgeRole::Admin)						? st::rankAdminFg->c
																		: st::rankUserFg->c;
			const auto badgeLeft = trect.left() + trect.width() - badge->width;
			if (badge->role != BadgeRole::User) {
