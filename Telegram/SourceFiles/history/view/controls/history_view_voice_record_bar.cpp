...
[[nodiscard]] QString FormatTrimDuration(crl::time duration) {
	...
	if ((result.size() == 5) && (result[0] == QChar('0')) && (result[2] == QChar(':'))) {
		result.remove(0, 1);
	}
	return result;
}
...
[[nodiscard]] VoiceWaveform ResampleWaveformToRange(const VoiceWaveform &source, float64 left, float64 right) {
	...
	for (auto i = 0; i != size; ++i) {
		...
		const auto from = std::clamp(int(std::floor(segmentStart)), 0, size - 1);
		const auto till = std::clamp(int(std::ceil(segmentEnd)) - 1, from, size - 1);
		auto peak = uchar(0);
...
[[nodiscard]] VoiceWaveform ResampleWaveformToSize(const VoiceWaveform &source, int targetSize) {
	...
	for (auto i = 0; i != targetSize; ++i) {
		...
		const auto from = std::clamp(int(std::floor(segmentStart)), 0, sourceSize - 1);
		const auto till = std::clamp(int(std::ceil(segmentEnd)) - 1, from, sourceSize - 1);
		auto peak = uchar(0);
		for (auto j = from; j <= till; ++j) {
			peak = std::max(peak, uchar(source[j]));
		}
...
[[nodiscard]] VoiceWaveform MergeWaveformsByDuration(const VoiceWaveform &first,
	...
	auto firstSize = int(((firstDuration * targetSize) + (totalDuration / 2)) / totalDuration);
...
[[nodiscard]] Ui::RoundVideoResult ToRoundVideoResult(::Media::Capture::Result &&data) {
...
TTLButton::TTLButton(not_null<Ui::RpWidget *> parent, const style::RecordBar &st, bool recordingVideo)
		...
		rpl::on_next(
			[=](bool shown)
			{
				...
				auto text = rpl::conditional(
					...
					(recordingVideo ? tr::lng_record_once_active_video : tr::lng_record_once_active_tooltip)(tr::rich),
					tr::lng_record_once_first_tooltip(tr::rich));
					...
					rpl::on_next(
						[=](const QRect &r)
						{
							...
							_tooltip->pointAt(
								...
								[=](QSize size)
								{
									return QPoint(r.left() - size.width() - st::defaultImportantTooltip.padding.left(),
												  r.top() + r.height() - size.height() - st::historyRecordTooltipSkip +
				...
				if (!Core::App().settings().ttlVoiceClickTooltipHidden()) {
					clicks() | rpl::take(1) |
						rpl::on_next([=] { Core::App().settings().setTtlVoiceClickTooltipHidden(true); },
									 _tooltip->lifetime());
	...
	paintRequest() |
		rpl::on_next(
			...
			lifetime());
	setAccessibleName((recordingVideo ? tr::lng_in_dlg_video_message_ttl : tr::lng_in_dlg_voice_message_ttl)(tr::now));
}
...
private:
	...
	[[nodiscard]] TrimGeometry computeTrimGeometry(const QRect &trimRect) const;
...
ListenWrap::ListenWrap(not_null<Ui::RpWidget *> parent,
	...
	: _parent(parent), _st(st), _send(send), _session(session), _document(DummyDocument(&session->data())),
	  ...
	  _allowTrim(allowTrim), _delete(base::make_unique_q<Ui::IconButton>(parent, _st.remove)), _durationFont(font),
	  _duration(FormatTrimDuration(_data->duration)), _durationWidth(_durationFont->width(_duration)),
	  ...
	  _inactiveWaveformBar(anim::with_alpha(_activeWaveformBar, st::historyRecordWaveformInactiveAlpha)),
	...
	_parent->paintRequest() |
		rpl::on_next(
			[=](const QRect &clip)
			{
				...
				const auto useSendAnimationCache = _useSendAnimationCache && !_sendAnimationCache.isNull();
...

				{
					...
					const auto bgCenterRect = bgRect.marginsRemoved(style::margins(radius, 0, radius, 0));
				...
				: computeTrimGeometry(bgCenterRect);
					...
					p.setPen(Qt::NoPen);
					p.setBrush(anim::with_alpha(_st.cancelActive->c, st::historyRecordWaveformOutsideAlpha));
					...
					if (canTrim() && !trimFrameRect.isEmpty()) {
						...
						if (activeBgRect.isEmpty()) {
							...
							if (activeBgRect.x() > bgRect.x()) {
								p.fillRect(
									QRect(bgRect.x(), bgRect.y(), activeBgRect.x() - bgRect.x(), bgRect.height()),
									_st.cancelActive);
							}
							if (rect::right(activeBgRect) < rect::right(bgRect)) {
								p.fillRect(QRect(rect::right(activeBgRect) + 1,
												 bgRect.y(),
												 rect::right(bgRect) - rect::right(activeBgRect),
												 bgRect.height()),
										   _st.cancelActive);
							}
						}
					...
					if (!waveformRect.isEmpty()) {
						...
						const auto paintWaveform =
							[&](QRect rect, float64 opacity, const QColor &activeBar, const QColor &inactiveBar)
						{
							...
							if (_data->minithumbs.isNull()) {
								...
								PaintWaveform(
									p, _voiceData.get(), waveformRect.width(), activeBar, inactiveBar, playProgress);
							} else {
								FillWithMinithumbs(p, _data, waveformRect, playProgress);
							}
							p.restore();
						};
						const auto activeWaveformRect = trimFrameRect.intersected(waveformRect);
						if (canTrim() && !activeWaveformRect.isEmpty()) {
							const auto outsideOpacity = std::clamp(st::historyRecordWaveformOutsideAlpha, 0., 1.);
							paintWaveform(activeWaveformRect, 1., _activeWaveformBar, _inactiveWaveformBar);
							paintWaveform(QRect(waveformRect.x(),
												waveformRect.y(),
												std::max(0, activeWaveformRect.x() - waveformRect.x()),
												waveformRect.height()),
										  outsideOpacity,
										  _inactiveWaveformBar,
										  _inactiveWaveformBar);
							paintWaveform(
								QRect(rect::right(activeWaveformRect) + 1,
									  waveformRect.y(),
									  std::max(0, rect::right(waveformRect) - rect::right(activeWaveformRect)),
									  waveformRect.height()),
								outsideOpacity,
								_inactiveWaveformBar,
								_inactiveWaveformBar);
						} else {
							paintWaveform(waveformRect, 1., _activeWaveformBar, _inactiveWaveformBar);
						}
						if (canTrim() && !trimFrameRect.isEmpty()) {
							p.setPen(Qt::NoPen);
							...
							{
								const auto width = std::min(inner.width(), handle.width());
								const auto height = std::min(inner.height(), handle.height());
								const auto x = handle.x() + (handle.width() - width) / 2;
								const auto y = handle.y() + (handle.height() - height) / 2;
								p.drawRoundedRect(QRect(x, y, width, height), width / 2., width / 2.);
							};
							...
							if (lineHeight > 0) {
								...
								if (rightLineX != leftLineX) {
									p.fillRect(rightLineX, lineTop, 1, lineHeight, _st.bg);
								}
							}
						}
						...
						if (!_controlRect.isEmpty()) {
							...
							p.drawRoundedRect(_controlRect, _controlRect.height() / 2., _controlRect.height() / 2.);
							...
							if (_controlHasDuration) {
								...
								const auto top = _controlRect.y() + (_controlRect.height() - ascent) / 2;
								p.drawText(QRect(left, top, _durationWidth, ascent), style::al_left, _duration);
							}
						}
					}
				}
...
void ListenWrap::initPlayButton() {
	...
	_playPauseButton->paintRequest() |
		rpl::on_next(
			[=](const QRect &clip)
			{
				...
				const auto progress = _showProgress.current() * st::historyRecordCenterControlIconScale;
	...
	_playPauseButton->setClickedCallback([=] { playPause(); });
	...
	showPause->changes() |
		rpl::on_next(
			[=](bool pause)
			{
				_playPauseButton->setAccessibleName(pause ? tr::lng_record_lock_pause(tr::now)
	...
	instance()->stops(AudioMsgId::Type::Voice) | rpl::on_next([=] { *showPause = false; }, _lifetime);
...
void ListenWrap::initPlayProgress() {
	...
	{ return isInPlayer() && _waveformFgRect.contains(p) && (_controlRect.isEmpty() || !_controlRect.contains(p)); };

	const auto updateCursor = [=](const QPoint &p)
	{
		if (canTrim() && (_trimLeftHandleRect.contains(p) || _trimRightHandleRect.contains(p))) {
		...
		rpl::on_next(
			[=](const State &state)
			{
				...
				const auto playbackTrimmed = canTrim() &&
					((leftBoundary > kTrimPlaybackEpsilon) || (rightBoundary < (1. - kTrimPlaybackEpsilon)));
				const auto length = int(state.length);
				const auto position = std::min(state.position, int64(length));
				auto progress = length ? Progress(position, length) : 0.;
				if (playbackTrimmed && length > 0) {
					if (ShowPauseIcon(state.state) && (progress < (leftBoundary - kTrimPlaybackEpsilon))) {
					...
					if (ShowPauseIcon(state.state) && (progress >= (rightBoundary - kTrimPlaybackEpsilon))) {
						instance()->stop(voice, true);
				...
				if (IsStopped(state.state)) {
					_playProgress = playbackTrimmed ? anim::value(leftBoundary, leftBoundary) : anim::value();
				} else {
	...
	_parent->events() |
		...
		rpl::on_next(
			[=](not_null<QEvent *> e)
			{
				...
				if ((*dragMode == DragMode::TrimLeft) || (*dragMode == DragMode::TrimRight)) {
					if (isRelease || (type == QEvent::MouseMove)) {
						const auto progress = trimProgressFromPosition(pos.x());
						const auto minDelta = minimumTrimProgress();
						if (*dragMode == DragMode::TrimLeft) {
							_trimLeftProgress = std::clamp(progress, 0., std::max(0., _trimRightProgress - minDelta));
						} else {
							_trimRightProgress = std::clamp(progress, std::min(1., _trimLeftProgress + minDelta), 1.);
						}
						updateDurationText();
						updateTrimGeometry();
						updateControlGeometry();
...
bool ListenWrap::canTrim() const { return _allowTrim; }
...
float64 ListenWrap::trimProgressFromPosition(int x) const {
	...
	return std::clamp(float64(x - _waveformBgFinalCenterRect.x()) / width, 0., 1.);
}

float64 ListenWrap::minimumTrimProgress() const {
	const auto samplesProgress = [&]
	{
		const auto samples = int((_data->duration * ::Media::Player::kDefaultFrequency) / crl::time(1000));
		if (samples <= 0) {
			return 0.;
		}
		return std::clamp(float64(kMinSamples) / samples, 0., 1.);
	}();
...
float64 ListenWrap::minimumControlTrimProgress() const {
	...
	const auto handleWidth = std::max(1, std::min(st::historyRecordTrimHandleWidth, trimRect.width() / 2));
	const auto previewRange = std::max(1, trimRect.width() - (handleWidth * 2));
	const auto controlHeight = std::min(st::historyRecordCenterControlHeight, trimRect.height());
	const auto iconWidth = controlHeight;
	const auto minControlWidth = (st::historyRecordCenterControlPadding * 2) + iconWidth +
...
std::optional<ListenWrap::TrimRange> ListenWrap::selectedTrimRange() const {
	...
	const auto currentSamples = int((_data->duration * ::Media::Player::kDefaultFrequency) / crl::time(1000));
	if (currentSamples <= 0) {
		return std::nullopt;
	}
	...
	const auto from = (fromSamples * crl::time(1000)) / ::Media::Player::kDefaultFrequency;
...
ListenWrap::TrimGeometry ListenWrap::computeTrimGeometry(const QRect &trimRect) const {
	...
	const auto handleWidth = std::max(1, std::min(st::historyRecordTrimHandleWidth, width / 2));
	...
	const auto rightProgress = std::clamp(_trimRightProgress, leftProgress, 1.);
	const auto leftBoundary =
		std::clamp(minBoundary + int(base::SafeRound(previewRange * leftProgress)), minBoundary, maxBoundary);
	const auto rightBoundary =
		std::clamp(minBoundary + int(base::SafeRound(previewRange * rightProgress)), leftBoundary, maxBoundary);
	result.leftHandle = QRect(leftBoundary - handleWidth, trimRect.y(), handleWidth, trimRect.height());
	result.rightHandle = QRect(rightBoundary, trimRect.y(), handleWidth, trimRect.height());
	const auto previewLeft = leftBoundary;
	const auto previewRight = std::max(previewLeft, rightBoundary - 1);
	result.frame = QRect(QPoint(previewLeft, trimRect.y()), QPoint(previewRight, rect::bottom(trimRect)));
	return result;
}

void ListenWrap::updateControlGeometry() {
	const auto availableRect = (canTrim() && !_trimFrameRect.isEmpty()) ? _trimFrameRect : _waveformBgFinalCenterRect;
	if (availableRect.isEmpty()) {
		_controlRect = QRect();
		_controlHasDuration = false;
		return;
	}
	const auto controlHeight = std::min(st::historyRecordCenterControlHeight, availableRect.height());
	const auto iconWidth = controlHeight;
	const auto iconOnlyWidth = (st::historyRecordCenterControlPadding * 2) + iconWidth;
	const auto fullWidth = iconOnlyWidth + st::historyRecordCenterControlTextSkip + _durationWidth;
	...
	_controlRect = QRect(availableRect.x() + (availableRect.width() - controlWidth) / 2,
						 availableRect.y() + (availableRect.height() - controlHeight) / 2,
						 controlWidth,
						 controlHeight);
	_playPauseButton->resize(iconWidth, controlHeight);
	const auto iconLeft = _controlHasDuration ? (_controlRect.x() + st::historyRecordCenterControlPadding)
											  : (_controlRect.x() + (_controlRect.width() - iconWidth) / 2);
	_playPauseButton->moveToLeft(iconLeft, _controlRect.y());
}
...
void ListenWrap::applyTrimSelection(bool resetSelection) {
	...
	auto waveform = ResampleWaveformToRange(_data->waveform, waveLeft, waveRight);
	const auto from = range->from;
	const auto till = range->till;
	const auto selected = till - from;
	const auto selectedSamples = int((selected * ::Media::Player::kDefaultFrequency) / crl::time(1000));
	...
	_voiceData->wavemax = _voiceData->waveform.empty() ? uchar(0) : *ranges::max_element(_voiceData->waveform);
	if (resetSelection) {
		_trimLeftProgress = 0.;
...
void ListenWrap::applyTrimBeforeSend() { applyTrimSelection(true); }

void ListenWrap::prepareForSendAnimation() {
	if (_waveformBgRect.isEmpty()) {
		return;
	}
	const auto cacheRect = _waveformBgRect - style::margins(_st.remove.width, 0, _send->width(), 0);
...
void ListenWrap::playPause() { ::Media::Player::instance()->playPause({_document, FullMsgId()}); }
...
void CancelButton::init() {
	_showProgress.value() | rpl::map(rpl::mappers::_1 > 0.) | rpl::distinct_until_changed() |
		rpl::on_next([=](bool hasProgress) { setVisible(hasProgress); }, lifetime());
...
void VoiceRecordBar::init() {
	...
	rpl::single() |
		...
		rpl::on_next([=] { orderControls(); }, lifetime());
	...
	_inField.changes() | rpl::on_next([=](bool value) { activeAnimate(value); }, lifetime());
	...
	const auto setLevelAsSend = [=]
	{
		...
		_level->clicks() | rpl::on_next([=] { stop(true); }, _recordingLifetime);

		rpl::single(false) | rpl::then(_level->actives()) |
			rpl::on_next([=](bool enter) { _inField = enter; }, _recordingLifetime);
	};
	...
	_paused.value() | rpl::distinct_until_changed() |
		rpl::on_next(
			[=](bool paused)
			{
				_lock->setAccessibleName(paused ? tr::lng_record_lock_resume(tr::now) : tr::lng_record_lock(tr::now));
				if (!paused) {
					return;
				}
	...
	_listenChanges.events() | rpl::filter([=] { return _listen != nullptr; }) |
		rpl::on_next(
			[=]
			{
				_listen->stopRequests() | rpl::take(1) | rpl::on_next([=] { hideAnimated(); }, _listen->lifetime());
...
void VoiceRecordBar::applyListenTrimForResume() {
	...
	_resumeFromTrimmedListen = (_data.duration != beforeDuration) || (_data.content.size() != beforeSize);
}
...
void VoiceRecordBar::initLockGeometry() {
	...
	rpl::merge(_lock->heightValue() | rpl::to_empty,
		...
		rpl::on_next([=] { updateLockGeometry(); }, lifetime());
	parent->geometryValue() | rpl::on_next([=] { updateTTLGeometry(TTLAnimationType::RightLeft, 1.); }, lifetime());
}
...
void VoiceRecordBar::startRecordingAndLock(bool round) {
	{
		...
		sendState.type = round ? Ui::SendButton::Type::Round : Ui::SendButton::Type::Record;
...
void VoiceRecordBar::startRecording() {
	...
	auto appearanceCallback = [=]
	{
		...
		instance()->updated() |
			rpl::on_next_error([=](const Update &update) { recordUpdated(update.level, update.samples); },
							   [=] { stop(false); },
							   _recordingLifetime);
		if (_videoRecorder) {
	...
	_lock->locks() | rpl::on_next([=] { stateOwned->reset(); }, state->lifetime);
	...
	_listenChanges.events_starting_with(rpl::empty_value()) | rpl::filter([=] { return _listen == nullptr; }) |
		rpl::on_next(
			[=]
			{
				auto keyFilterCallback = [=](not_null<QEvent *> e)
				{
					...
					if (_send->type() != Ui::SendButton::Type::Record && _send->type() != Ui::SendButton::Type::Round) {
						return Result::Continue;
					}
					switch (e->type()) {
						case QEvent::KeyPress: {
							if (!_warningShown && isRecordingLocked() &&
								Ui::ShouldSubmit(static_cast<QKeyEvent *>(e.get()),
				...
				_keyFilterInRecordingState = base::unique_qptr{
					base::install_event_filter(QCoreApplication::instance(), std::move(keyFilterCallback)).get()};
			},
			lifetime());
}
...
int VoiceRecordBar::samplesFromDuration(crl::time duration) const {
	return int((duration * ::Media::Player::kDefaultFrequency) / crl::time(1000));
}

Ui::RoundVideoResult VoiceRecordBar::mergeWithResumePrefix(Ui::RoundVideoResult data) {
	...
	const auto tail = (_resumeRawDuration > 0)
		? ::Media::TrimAudioToRange(data.content, _resumeRawDuration, data.duration)
		: ::Media::AudioEditResult();
	if ((_resumeRawDuration > 0) && tail.content.isEmpty()) {
		return _resumePrefixData;
	}
	const auto combined =
		::Media::ConcatAudio(_resumePrefixData.content, (_resumeRawDuration > 0) ? tail.content : data.content);
	if (combined.content.isEmpty()) {
		return _resumePrefixData;
	}
	const auto tailDuration = (_resumeRawDuration > 0) ? tail.duration : data.duration;
	const auto duration = combined.duration ? combined.duration : (_resumePrefixData.duration + tailDuration);
	auto waveform = MergeWaveformsByDuration(_resumePrefixData.waveform,
					...
					crl::on_main(weak,
								 [=, data = std::move(data)]() mutable
								 {
									 ...
									 _listen = std::make_unique<ListenWrap>(
										 this, _st, _send, &_show->session(), &_data, false, _cancelFont);
									 _listenChanges.fire({});
			...
			instance()->pause(true,
							  crl::guard(this,
										 [=](Result &&data)
										 {
											 ...
											 auto combined = mergeWithResumePrefix(ToRoundVideoResult(std::move(data)));
											 clearResumePrefix();
											 if (combined.content.isEmpty()) {
												 // Close everything.
												 stop(false);
											 ...
											 window()->activateWindow();
											 _listen = std::make_unique<ListenWrap>(
												 this, _st, _send, &_show->session(), &_data, true, _cancelFont);
											 _listenChanges.fire({});
										 }));
		}
...
bool VoiceRecordBar::createVideoRecorder() {
	...
	_videoRecorder->placeholderUpdates() |
		rpl::on_next([=](QImage &&placeholder) { _show->session().local().writeRoundPlaceholder(placeholder); },
					 _videoCapturerLifetime);
