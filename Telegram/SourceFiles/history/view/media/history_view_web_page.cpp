...
[[nodiscard]] TextWithEntities PageToPhrase(not_null<WebPageData *> page) {
	...
	const auto text =
		tr::upper(page->iv								 ? tr::lng_view_button_iv(tr::now)
					  ...
					  : (type == WebPageType::ConferenceCall) ? tr::lng_view_button_call(tr::now)
					  ...
					  : (type == WebPageType::StoryAlbum)				? tr::lng_view_button_storyalbum(tr::now)
					  : (type == WebPageType::GiftCollection)			? tr::lng_view_button_collection(tr::now)
					  : (type == WebPageType::Auction)
					  ? ((page->auction && page->auction->endDate && page->auction->endDate <= base::unixtime::now())
							 ? tr::lng_auction_preview_view_results(tr::now)
							 : (page->auction && page->auction->auctionGift->auctionStartDate &&
								(page->auction->auctionGift->auctionStartDate > base::unixtime::now()))
...
[[nodiscard]] bool HasButton(not_null<WebPageData *> webpage) {
	...
	return webpage->iv || webpage->uniqueGift || (type == WebPageType::Message) || (type == WebPageType::Group) ||
		...
		(type == WebPageType::ConferenceCall) || (type == WebPageType::BotApp) ||
		...
		(type == WebPageType::StickerSet) || (type == WebPageType::StoryAlbum) ||
		(type == WebPageType::GiftCollection) || (type == WebPageType::Auction);
}
...
WebPage::WebPage(not_null<Element *> parent, not_null<WebPageData *> data, MediaWebPageFlags flags)
	: Media(parent), _st(data->type == WebPageType::Factcheck ? st::factcheckPage : st::historyPagePreview),
	  _data(data), _flags(flags), _siteName(st::minPhotoSize - rect::m::sum::h(_st.padding)),
...
QSize WebPage::countOptimalSize() {
	...
	if (HasButton(_data)) {
		...
		_openButton.setText(st::semiboldTextStyle, tr::upper(sponsored->buttonText));
	}
	...
	if (!_attach && _data->uniqueGift) {
		_attach = std::make_unique<MediaGeneric>(
			...
			MediaGenericDescriptor{
				...
				.paintBgFactory = [=] { return UniqueGiftBg(_parent, _data->uniqueGift); },
				.expandCurrentWidth = true,
			});
	} else if (!_attach && _data->auction) {
		const auto &gift = _data->auction->auctionGift;
		const auto backdrop = gift->background ? gift->background->backdrop() : Data::UniqueGiftBackdrop();
		_attach = std::make_unique<MediaGeneric>(_parent,
												 GenerateAuctionPreview(_parent, nullptr, gift, backdrop),
												 MediaGenericDescriptor{
													 .maxWidth = st::msgServiceGiftPreview,
													 .paintBgFactory =
														 [=]
													 {
														 return AuctionBg(_parent,
																		  backdrop,
																		  gift,
	...
	// init strings
	if (_description.isEmpty() && !_data->description.text.isEmpty() && !_data->uniqueGift && !_data->auction) {
	...
	if (!siteName.isEmpty()) {
		_siteNameLines = 1;
		_siteName.setMarkedText(st::webPageTitleStyle, tr::link(siteName, _data->url), Ui::WebpageTextTitleOptions());
	}
	if (_title.isEmpty() && !title.isEmpty()) {
		if (!_siteNameLines && !_data->url.isEmpty()) {
			_title.setMarkedText(st::webPageTitleStyle, tr::link(title, _data->url), Ui::WebpageTextTitleOptions());
...
void WebPage::draw(Painter &p, const PaintContext &context) const {
	...
	const auto &colorCollectible = factcheck   ? nullptr
		: (sponsored && sponsored->colorIndex) ? nullptr
											   : view->contentColorCollectible();
	const auto colorPattern =
		colorCollectible ? st->collectiblePatternIndex(colorCollectible) : st->colorPatternIndex(colorIndex);
	const auto useColorCollectible = colorCollectible && !context.outbg;
	const auto useColorIndex = !context.outbg;
	const auto cache = useColorCollectible ? st->collectibleReplyCache(selected, colorCollectible).get()
		: useColorIndex					   ? st->coloredReplyCache(selected, colorIndex).get()
	...
	const auto backgroundEmojiCache = !backgroundEmojiData ? nullptr
		: useColorCollectible ? &backgroundEmojiData->collectibleCaches[colorCollectible]
							  : &backgroundEmojiData->caches[Ui::BackgroundEmojiData::CacheIndex(
									selected, context.outbg, true, useColorIndex ? (colorIndex + 1) : 0)];
	...
	if (!settings.simpleQuotesAndReplies && backgroundEmojiData) {
		...
		if (!backgroundEmojiCache->frames[0].isNull()) {
			FillBackgroundEmoji(p, outer, false, *backgroundEmojiCache, backgroundEmojiData->firstGiftFrame);
		}
	...
	if (_siteNameLines) {
		p.setPen(cache->icon);
		p.setTextPalette(useColorCollectible ? st->collectibleTextPalette(selected, colorCollectible)
							 : useColorIndex ? st->coloredTextPalette(selected, colorIndex)
											 : stm->semiboldPalette);
...
bool WebPage::allowsNarrowBubble() const {
	return (_attach != nullptr) && (_data->uniqueGift != nullptr || _data->auction != nullptr);
}

int WebPage::minBubbleWidthForNarrowBubble() const { return allowsNarrowBubble() ? maxWidth() : 0; }
