...
void AddForwardToSavedAction(not_null<Ui::PopupMenu *> menu,
							 const ContextMenuRequest &request,
							 not_null<ListWidget *> list) {
	const auto item = request.item;
	...
	menu->addAction(
		QString("Forward to Saved Messages"),
		[=]
		{
			if (const auto item = owner->message(itemId)) {
				const auto self = session->user();
				const auto history = session->data().history(self);
				auto items = (asGroup ? owner->itemOrItsGroup(item) : MessageIdsList{1, itemId});
