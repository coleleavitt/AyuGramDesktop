...
HistoryInner::HistoryInner(not_null<HistoryWidget *> historyWidget,
	...
	session().data().viewLayoutChanged() |
		...
		rpl::on_next([=](not_null<const Element *> view) { mouseActionUpdate(); }, lifetime());
	...
	controller->adaptive().chatWideValue() | rpl::on_next([=](bool wide) { _isChatWide = wide; }, lifetime());
			...
			texts
				.emplace(item->position(),
						 Part{
							 ...
							 .time = QString("[%1] ").arg(QLocale().toString(ItemDateTime(item), QLocale::ShortFormat)),
