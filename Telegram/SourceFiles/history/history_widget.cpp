...
HistoryWidget::HistoryWidget(QWidget *parent, not_null<Window::SessionController *> controller)
	: Window::AbstractSectionWidget(parent, controller, ActivePeerValue(controller)),
	  ...
	  _fieldBarCancel(this, st::historyReplyCancel), _topBars(std::make_unique<Ui::RpWidget>(this)),
	  _topBar(this, controller), _scroll(this, controller->chatStyle()->value(lifetime(), st::historyScroll), false),
	...
	session().downloaderTaskFinished() | rpl::on_next([=] { update(); }, lifetime());
	...
	_scroll->scrolls() | rpl::on_next([=] { handleScroll(); }, lifetime());
	_scroll->geometryChanged() | rpl::on_next(crl::guard(_list, [=] { _list->onParentGeometryChanged(); }), lifetime());
	...
	_mediaEditManager.updateRequests() |
		rpl::on_next([this] { updateOverStates(mapFromGlobal(QCursor::pos())); }, lifetime());
	...
	_field->submits() |
		rpl::on_next([=](Qt::KeyboardModifiers modifiers) { sendWithModifiers(modifiers); }, _field->lifetime());
	_field->cancelled() |
		rpl::on_next(
			[=]
			{
	...
	_field->tabbed() |
		rpl::on_next(
			...
			_field->lifetime());
	_field->heightChanges() | rpl::on_next([=] { fieldResized(); }, _field->lifetime());
	_field->focusedChanges() | rpl::filter(rpl::mappers::_1) |
		rpl::on_next([=] { fieldFocused(); }, _field->lifetime());
	_field->changes() | rpl::on_next([=] { fieldChanged(); }, _field->lifetime());
#ifdef Q_OS_MAC
	// Removed an ability to insert text from the menu bar
	// when the field is hidden.
	_field->shownValue() | rpl::on_next([=](bool shown) { _field->setEnabled(shown); }, _field->lifetime());
#endif // Q_OS_MAC
	controller->widget()->shownValue() | rpl::skip(1) | rpl::on_next([=] { windowIsVisibleChanged(); }, lifetime());
	...
	rpl::merge(_field->scrollTop().changes() | rpl::to_empty,
		...
		rpl::on_next([=] { saveDraftDelayed(); }, _field->lifetime());
	...
	controller->chatStyle()->paletteChanged() | rpl::on_next([=] { _scroll->updateBars(); }, lifetime());
	...
	_keyboard->sendCommandRequests() | rpl::on_next([=](Bot::SendCommandRequest r) { sendBotCommand(r); }, lifetime());
	...
	rpl::merge(
		...
		rpl::on_next([=] { refreshAttachBotsMenu(); }, lifetime());
	...
	session().data().newItemAdded() |
		rpl::on_next([=](not_null<HistoryItem *> item) { newItemAdded(item); }, lifetime());

	session().data().historyChanged() |
		rpl::on_next([=](not_null<History *> history) { handleHistoryChange(history); }, lifetime());
	...
	session().data().itemDataChanges() |
		...
		rpl::on_next([=](not_null<HistoryItem *> item) { item->mainView()->itemDataChanged(); }, lifetime());

	rpl::merge(session().changes().peerUpdates(Data::PeerUpdate::Flag::IsBlocked) | rpl::to_empty,
	...
	session().data().drawToReplyRequests() |
		rpl::on_next([=](Data::DrawToReplyRequest request) { handleDrawToReplyRequest(std::move(request)); },
					 lifetime());

	Core::App().settings().largeEmojiChanges() |
		rpl::on_next([=] { crl::on_main(this, [=] { updateHistoryGeometry(); }); }, lifetime());
	Core::App().settings().sendSubmitWayValue() |
		rpl::on_next([=] { crl::on_main(this, [=] { updateFieldSubmitSettings(); }); }, lifetime());
	...
	session().changes().realtimeMessageUpdates(MessageUpdateFlag::NewUnreadReaction) |
		rpl::on_next([=](const Data::MessageUpdate &update) { maybeMarkReactionsRead(update.item); }, lifetime());
	...
	Media::Player::instance()->switchToNextEvents() |
		...
		rpl::on_next([=](const MediaSwitch &pair) { scrollToCurrentVoiceMessage(pair.from.contextId(), pair.to); },
					 lifetime());
	...
	session().changes().peerUpdates(
		...
		PeerUpdateFlag::Notifications | PeerUpdateFlag::ChannelAmIn | PeerUpdateFlag::DiscussionLink |
		PeerUpdateFlag::Slowmode | PeerUpdateFlag::BotStartToken | PeerUpdateFlag::MessagesTTL |
		PeerUpdateFlag::ChatThemeToken | PeerUpdateFlag::FullInfo | PeerUpdateFlag::StarsPerMessage |
		...
		rpl::filter(
			[=](const Data::PeerUpdate &update)
			{
				if (update.peer.get() == _peer) {
					return true;
				} else if (update.peer->isSelf() && (update.flags & PeerUpdateFlag::GiftSettings)) {
		...
		rpl::on_next(
			[=](Data::PeerUpdate::Flags flags)
			{
				...
				if (flags & (PeerUpdateFlag::BotStartToken | PeerUpdateFlag::GiftSettings)) {
					updateControlsVisibility();
					updateControlsGeometry();
				}
				...
				if ((flags & PeerUpdateFlag::ChatThemeToken) && _list) {
					...
					if (Data::CloudThemes::TestingColors() && !emoji.isEmpty()) {
						_peer->owner().cloudThemes().themeForTokenValue(emoji) | rpl::filter_optional() | rpl::take(1) |
							rpl::on_next(
	...
	rpl::merge(session().data().notifySettings().defaultUpdates(Type::User),
		...
		rpl::on_next([=] { updateNotifyControls(); }, lifetime());
	...
	_topBar->membersShowAreaActive() |
		rpl::on_next([=](bool active) { setMembersShowAreaActive(active); }, _topBar->lifetime());
	_topBar->forwardSelectionRequest() | rpl::on_next([=] { forwardSelected(); }, _topBar->lifetime());
	_topBar->deleteSelectionRequest() | rpl::on_next([=] { confirmDeleteSelected(); }, _topBar->lifetime());
	_topBar->messageShotSelectionRequest() | rpl::on_next([=] { messageShotSelected(); }, _topBar->lifetime());
	_topBar->clearSelectionRequest() | rpl::on_next([=] { clearSelected(); }, _topBar->lifetime());
	_topBar->cancelChooseForReportRequest() |
		rpl::on_next([=] { setChooseReportMessagesDetails({}, nullptr); }, _topBar->lifetime());
	...
	session().api().sendActions() |
		rpl::filter(
			...
			{
				if (_creatingBotTopic && action.history == _creatingBotTopic->owningHistory() &&
					...
					Ui::PostponeCall(_creatingBotTopic,
									 [=]
									 {
										 ...
										 controller->showSection(std::make_shared<ChatMemento>(ChatViewId{
		...
		rpl::on_next(
			[=](const Api::SendAction &action)
			{
				...
				if (action.replaceMediaOf) {
					...
					if (!_justMarkingAsRead && cancelReplyOrSuggest(lastKeyboardUsed) && !action.clearDraft) {
						saveCloudDraft();
					}
				}
	...
	_selfForwardsTagger = std::make_unique<HistoryView::SelfForwardsTagger>(
		controller, this, [=] { return _list; }, _scroll.data(), [=] { return _history; });

	if (session().supportMode()) {
		session().data().chatListEntryRefreshes() |
			rpl::on_next([=] { crl::on_main(this, [=] { checkSupportPreload(true); }); }, lifetime());
	}

	Core::App().materializeLocalDraftsRequests() | rpl::on_next([=] { saveFieldToHistoryLocalDraft(); }, lifetime());
...
void HistoryWidget::refreshDirectMessageShown() {
	...
	if (visible) {
		...
		_directMessageLifetime = monoforum->flagsValue() | rpl::skip(1) |
			rpl::on_next(
									 [=](Flags::Change change)
									 {
...
void HistoryWidget::initVoiceRecordBar() {
	...
	_voiceRecordBar->sendVoiceRequests() | rpl::on_next([=](const VoiceToSend &data) { sendVoice(data); }, lifetime());

	_voiceRecordBar->cancelRequests() | rpl::on_next(applyLocalDraft, lifetime());
	...
	_voiceRecordBar->updateSendButtonTypeRequests() | rpl::on_next([=] { updateSendButtonType(); }, lifetime());

	_voiceRecordBar->lockViewportEvents() |
		rpl::on_next([=](not_null<QEvent *> e) { _scroll->viewportEvent(e); }, lifetime());
...
void HistoryWidget::initTabbedSelector() {
	...
	selector->emojiChosen() | rpl::filter([=] { return !isHidden() && !_field->isHidden(); }) |
		rpl::on_next([=](ChatHelpers::EmojiChosen data) { Ui::InsertEmojiAtCursor(_field->textCursor(), data.emoji); },
					 lifetime());

	rpl::merge(selector->fileChosen() | filter,
			   selector->customEmojiChosen() | filter,
			   controller()->stickerOrEmojiChosen() | filter) |
		rpl::on_next([=](ChatHelpers::FileChosen &&data) { fileChosen(std::move(data)); }, lifetime());

	selector->photoChosen() | filter |
		rpl::on_next([=](ChatHelpers::PhotoChosen data) { sendExistingPhoto(data.photo, data.options); }, lifetime());
	...
	selector->inlineResultChosen() | filter |
		...
		rpl::on_next([=](ChatHelpers::InlineChosen data) { sendInlineResult(data); }, lifetime());

	selector->contextMenuRequested() | filter |
		rpl::on_next([=] { selector->showMenuWithDetails(sendMenuDetails()); }, lifetime());
...
void HistoryWidget::supportInitAutocomplete() {
	...
	_supportAutocomplete->insertRequests() |
		rpl::on_next([=](const QString &text) { supportInsertText(text); }, _supportAutocomplete->lifetime());

	_supportAutocomplete->shareContactRequests() |
		rpl::on_next([=](const Support::Contact &contact) { supportShareContact(contact); },
...
void HistoryWidget::supportShareContact(Support::Contact contact) {
	...
	box->boxClosing() | rpl::on_next([=] { _field->document()->undo(); }, lifetime());
}
...
int HistoryWidget::itemTopForHighlight(not_null<HistoryView::Element *> view) const {
	...
	if (heightLeft >= 0) {
		return std::max(itemTop - (heightLeft / 2), 0);
			   ...
			   (!highlight.range.empty() || highlight.todoItemId) && !IsSubGroupSelection(highlight.range)) {
		const auto sel = highlight.range;
		const auto single = st::messageTextStyle.font->height;
		const auto todoy = sel.empty() ? HistoryView::FindViewTaskY(view, highlight.todoItemId) : 0;
		const auto begin = sel.empty() ? (todoy - 4 * single) : HistoryView::FindViewY(view, sel.from) - single;
		const auto end =
			sel.empty() ? (todoy + 4 * single) : (HistoryView::FindViewY(view, sel.to, begin + single) + 2 * single);
		auto result = itemTop;
	...
	if (bot) {
		...
		if (!_inlineResults) {
				...
				[=](InlineBots::ResultSelected result)
				{
					if (result.open) {
						...
						if (const auto photo = request.photo()) {
							controller()->openPhoto(photo, {.showDrawButton = showDrawButton});
						} else if (const auto document = request.document()) {
							controller()->openDocument(document, false, {.showDrawButton = showDrawButton});
						}
					} else {
			...
			_inlineResults->requesting() |
				rpl::on_next([=](bool requesting) { _tabbedSelectorToggle->setLoading(requesting); },
...
void HistoryWidget::toggleChooseChatTheme(not_null<PeerData *> peer, std::optional<bool> show) {
	...
	_chooseTheme->shouldBeShownValue() | rpl::on_next(update, _chooseTheme->lifetime());
}
...
void HistoryWidget::saveFieldToHistoryLocalDraft() {
	...
	if (_editMsgId) {
		...
		if (_replyTo || suggest.exists || !_field->empty()) {
			_history->setLocalDraft(std::make_unique<Data::Draft>(_field, _replyTo, suggest, _preview->draft()));
		} else {
	...
	Shortcuts::Requests() |
		...
		rpl::on_next(
			[=](not_null<Shortcuts::Request *> request)
			{
				...
				if (showRecordButton() && _canSendMessages && _joinChannel->isHidden() && !_composeSearch) {
					const auto isVoice = request->check(Command::RecordVoice, 1);
					const auto isRound = !isVoice && request->check(Command::RecordRound, 1);
					(isVoice || isRound) &&
						request->handle(
							[=]
							{
				...
				const auto hasRecentActions = channel && (channel->hasAdminRights() || channel->amCreator());
				if (hasRecentActions) {
					request->check(Command::ShowAdminLog, 1) &&
						request->handle(
							[=]
							{
								controller()->showSection(std::make_shared<AdminLog::SectionMemento>(channel));
								return true;
							});
				}
				if (session().supportMode()) {
...
void HistoryWidget::setupGiftToChannelButton() {
	_giftToChannel = Ui::CreateChild<Ui::IconButton>(_muteUnmute.data(), st::historyGiftToChannel);
	_giftToChannel->setAccessibleName(tr::lng_gift_channel_title(tr::now));
	widthValue() |
		rpl::on_next([=](int width) { _giftToChannel->moveToRight(0, 0, width); }, _giftToChannel->lifetime());
	_giftToChannel->setClickedCallback([=] { Ui::ShowStarGiftBox(controller(), _peer); });
	rpl::combine(_muteUnmute->shownValue(), _joinChannel->shownValue()) |
		rpl::on_next(
			[=](bool muteUnmute, bool joinChannel)
			{
				const auto newParent = (muteUnmute && !joinChannel) ? _muteUnmute.data()
					: (joinChannel && !muteUnmute)					? _joinChannel.data()
																	: nullptr;
				if (newParent) {
...
void HistoryWidget::setupDirectMessageButton() {
	_directMessage = Ui::CreateChild<Ui::IconButton>(_muteUnmute.data(), st::historyDirectMessage);
	_directMessage->setAccessibleName(tr::lng_profile_direct_messages(tr::now));
	widthValue() |
		rpl::on_next([=](int width) { _directMessage->moveToLeft(0, 0, width); }, _directMessage->lifetime());
		...
		[=]
		{
			if (const auto channel = _peer ? _peer->asChannel() : nullptr) {
				if (channel->invitePeekExpires()) {
					...
					controller()->showPeerHistory(monoforum, Window::SectionShow::Way::Forward);
				}
			}
		});
	rpl::combine(_muteUnmute->shownValue(), _joinChannel->shownValue()) |
...
bool HistoryWidget::applyDraft(FieldHistoryAction fieldHistoryAction) {
	...
	const auto editDraft = _history ? _history->localEditDraft(MsgId(), PeerId()) : nullptr;
	const auto draft = editDraft ? editDraft : _history ? _history->localDraft(MsgId(), PeerId()) : nullptr;
...
void HistoryWidget::showHistory(PeerId peerId, MsgId showAtMsgId, const Window::SectionShow &params) {
	...
	if (peerId) {
		...
		_contactStatus = std::make_unique<ContactStatus>(controller(), _topBars.get(), _peer, false);
		_contactStatus->bar().heightValue() |
			rpl::on_next([=] { updateControlsGeometry(); }, _contactStatus->bar().lifetime());

		refreshGiftToChannelShown();
		refreshDirectMessageShown();
		if (const auto user = _peer->asUser()) {
			_paysStatus = std::make_unique<PaysStatus>(controller(), _topBars.get(), user);
			_paysStatus->bar().heightValue() |
				rpl::on_next([=] { updateControlsGeometry(); }, _paysStatus->bar().lifetime());
			_businessBotStatus = std::make_unique<BusinessBotStatus>(controller(), _topBars.get(), user);
			_businessBotStatus->bar().heightValue() |
				rpl::on_next([=] { updateControlsGeometry(); }, _businessBotStatus->bar().lifetime());
		}
		orderWidgets();
	...
	if (_peer) {
		...
		if (const auto user = _peer->asUser()) {
			if (const auto &info = user->botInfo) {
				...
				Info::Profile::BirthdayValue(user) | rpl::map(Data::IsBirthdayTodayValue) | rpl::flatten_latest() |
...
void HistoryWidget::setHistory(History *history) {
	...
	if (_history) {
		...
		if (const auto wasCreatingBotTopic = base::take(_creatingBotTopic)) {
			wasCreatingBotTopic->forum()->discardCreatingId(wasCreatingBotTopic->rootId());
		}
	}
	if (history) {
		_history = history;
...
void HistoryWidget::setupPreview() {
	...
	_preview->repaintRequests() | rpl::on_next([=] { updateField(); }, _preview->lifetime());
...
void HistoryWidget::refreshAttachBotsMenu() {
	...
	_attachBotsMenu->heightValue() | rpl::on_next([=] { moveFieldControls(); }, _attachBotsMenu->lifetime());
}
...
void HistoryWidget::unregisterDraftSources() {
	...
	session().local().unregisterDraftSource(_history, Data::DraftKey::Local(MsgId(), PeerId()));
	session().local().unregisterDraftSource(_history, Data::DraftKey::LocalEdit(MsgId(), PeerId()));
}
...
void HistoryWidget::registerDraftSource() {
	...
	session().local().registerDraftSource(
		_history,
		(editMsgId ? Data::DraftKey::LocalEdit(MsgId(), PeerId()) : Data::DraftKey::Local(MsgId(), PeerId())),
...
bool HistoryWidget::updateReplaceMediaButton() {
	...
	_replaceMedia->setAccessibleName(_canReplaceMedia ? tr::lng_attach_replace(tr::now) : tr::lng_attach(tr::now));
...
void HistoryWidget::refreshSendGiftToggle() {
	...
	const auto has = user && _canSendMessages && !user->isServiceUser() && !user->isSelf() && !user->isBot() &&
		((disallowed & Type::SendHide) || (session().user()->disallowedGiftTypes() & Type::SendHide) ||
	...
	if (!_giftToUser && has) {
		...
		_giftToUser->show();
		_giftToUser->addClickHandler([=] { Ui::ShowStarGiftBox(controller(), _peer); });
		orderWidgets(); // Raise drag areas to the top.
...
void HistoryWidget::applySuggestOptions(SuggestOptions suggest, HistoryView::SuggestMode mode) {
	Expects(suggest.exists);

	using namespace HistoryView;
	_suggestOptions = std::make_unique<SuggestOptionsBar>(controller()->uiShow(), _peer, suggest, mode);
	_suggestOptions->updates() |
		rpl::on_next(
			[=]
			{
				updateField();
...
void HistoryWidget::setupSendAsToggle() {
	session().sendAsPeers().updated() |
		rpl::filter([=](Main::SendAsKey key)
					{ return (key.peer == _peer) && (key.type == Main::SendAsType::Message); }) |
		rpl::on_next(
			[=]
			{
				refreshSendAsToggle();
...
void HistoryWidget::showAboutTopPromotion() {
	...
	const auto text = type.isEmpty() ? tr::lng_proxy_sponsor_about(tr::now, tr::rich)
		: custom.isEmpty()			 ? tr::lng_about_psa_default(tr::now, tr::rich)
									 : tr::rich(custom);
...
void HistoryWidget::handleDrawToReplyRequest(Data::DrawToReplyRequest request) {
	if (!_peer || request.messageId.peer != _peer->id) {
		return;
	}
	auto image = HistoryView::ResolveDrawToReplyImage(&session().data(), request);
	if (image.isNull()) {
		return;
	}
	const auto replyTo = request.messageId;
	HistoryView::OpenDrawToReplyEditor(controller(),
									   std::move(image),
									   crl::guard(this,
												  [=](QImage &&result)
												  {
													  ...
													  auto list = Storage::PrepareMediaFromImage(
														  std::move(result), QByteArray(), st::sendMediaPreviewSize);
...
void HistoryWidget::unreadCountUpdated() {
	if (_history->unreadMark() || (_migrated && _migrated->unreadMark())) {
		...
		_cornerButtons.updateJumpDownVisibility(hideCounter ? 0
...
void HistoryWidget::saveEditMessage(Api::SendOptions options) {
	...
	if (item->computeSuggestionActions() == SuggestionActions::AcceptAndDecline) {
		...
		const auto checked = checkSendPayment(1 + int(_forwardPanel->items().size()), options, withPaymentApproved);
		if (!checked) {
			return;
		}
	}
	...
	_saveEditMsgRequestId =
		Api::EditTextMessage(item, sending, webPageDraft, options, done, fail, _mediaEditManager.spoilered());
}
...
Api::SendAction HistoryWidget::prepareSendAction(Api::SendOptions options) {
			...
			if (readyRootId) {
				...
				if (!_creatingBotTopic) {
					...
					if (!draft.ids.empty()) {
						...
						_history->setForwardDraft(_creatingBotTopic->rootId(), PeerId(), std::move(draft));
					}
				}
...
void HistoryWidget::sendVoice(const VoiceToSend &data) {
	...
	const auto checked = checkSendPayment(1 + int(_forwardPanel->items().size()), action.options, withPaymentApproved);
	if (!checked) {
		return;
	}
...
void HistoryWidget::send(Api::SendOptions options) {
	...
	if (!settings.sendReadMessages && !AyuSettings::isGhostExempt(_history->peer->id.value) &&
		settings.markReadAfterAction && lastMessage) {
		readHistory(lastMessage);
	}
	...
	if (showSendMessageError(
			message.textWithTags, ignoreSlowmodeCountdown, withPaymentApproved, message.action.options)) {
		return;
	}
...
bool HistoryWidget::checkSendPayment(int messagesCount, Api::SendOptions options, Fn<void(int)> withPaymentApproved) {
	return _peer && _sendPayment.check(controller(), _peer, options, messagesCount, std::move(withPaymentApproved));
}
				...
				controller()->show(Box(
					[=](not_null<Ui::GenericBox *> box)
					{
						...
						box->addRow(object_ptr<Ui::FlatLabel>(
							box, tr::lng_gigagroup_suggest_text() | rpl::map(tr::rich), st::infoAboutGigagroup));
						box->addButton(tr::lng_gigagroup_suggest_more(), AboutGigagroupCallback(group, controller()));
...
void HistoryWidget::updateOverStates(QPoint pos) {
	...
	const auto inDetails =
		detailsRect.contains(pos) && (_editMsgId || replyTo() || isReadyToForward || hasWebPage || _suggestOptions);
...
void HistoryWidget::sendBotCommand(const Bot::SendCommandRequest &request, Api::SendOptions options) {
	...
	const auto checked = checkSendPayment(1, action.options, withPaymentApproved);
	if (!checked) {
		return;
	}
...
void HistoryWidget::updateSendButtonType() {
	...
	const auto forbidden = [&]
	{
		...
		const auto restriction =
			(type == Type::Record) ? ChatRestriction::SendVoiceMessages : ChatRestriction::SendVideoMessages;
...
bool HistoryWidget::searchInChatEmbedded(QString query, Dialogs::Key chat, PeerData *searchFrom) {
	...
	if (!peer || ((Window::SeparateId(peer) != controller()->windowId()) && !controller()->isPrimary())) {
		return false;
	} else if (_peer != peer) {
		const auto weak = base::make_weak(this);
...
void HistoryWidget::showMembersDropdown() {
	...
	if (!_membersDropdown) {
		...
		_membersDropdown->setOwnedWidget(object_ptr<HistoryView::GroupMembersWidget>(this, controller(), _peer));
...
void HistoryWidget::moveFieldControls() {
	...
	// (_attachDocument|_attachPhoto) _field (_ttlInfo) (_scheduled) (_giftToUser) (_silent|_cmdStart|_kbShow)
	// (_toggleSuggestPost) (_kbHide|_tabbedSelectorToggle) _send
	...
	_field->setPlaceholder(
		[&]() -> rpl::producer<QString>
		{
			...
			if (_editMsgId) {
				...
				return tr::lng_message_stars_ph(lt_count, rpl::single(stars * 1.));
...
bool HistoryWidget::showSendingFilesError(const Ui::PreparedBundle &bundle) const {
...
bool HistoryWidget::showSendMessageError(const TextWithTags &textWithTags,
	...
	return withPaymentApproved && !checkSendPayment(request.messagesCount, options, withPaymentApproved);
}
...
bool HistoryWidget::confirmSendingFiles(Ui::PreparedList &&list, const QString &insertTextOnCancel) {
	...
	box->setConfirmedCallback(crl::guard(this,
										 [=](std::shared_ptr<Ui::PreparedBundle> bundle, Api::SendOptions options)
	...
	box->takeTextWithTagsRequests() |
		rpl::on_next([=](TextWithTags &&text) { _field->setTextWithTags(std::move(text)); }, box->lifetime());
...
void HistoryWidget::sendingFilesConfirmed(std::shared_ptr<Ui::PreparedBundle> bundle, Api::SendOptions options) {
	...
	const auto checked = checkSendPayment(bundle->totalCount, action.options, withPaymentApproved);
	if (!checked) {
		return;
	}
...
void HistoryWidget::updateControlsGeometry() {
	...
	const auto tabsLeftSkip = _subsectionTabs ? _subsectionTabs->leftSkip() : 0;
	...
	const auto scrollAreaTop =
		_topBars->y() + businessBotTop + (_businessBotStatus ? _businessBotStatus->bar().height() : 0);
	_topBars->resize(innerWidth, scrollAreaTop - _topBars->y() + st::lineWidth);
...
FullReplyTo HistoryWidget::replyTo() const {
	return _replyTo									   ? _replyTo
		...
		: (_peer && _peer->forum() && !_peer->isBot()) ? FullReplyTo{.topicRootId = Data::ForumTopic::kGeneralId}
													   : FullReplyTo();
}

SuggestOptions HistoryWidget::suggestOptions(bool skipNoAdminCheck) const {
	const auto checked = skipNoAdminCheck || (_history && _history->suggestDraftAllowed());
	return (checked && _suggestOptions) ? _suggestOptions->values() : SuggestOptions();
}
...
void HistoryWidget::updateSendRestriction() {
	...
	if (AyuForward::isForwarding(_peer->id)) {
		...
		const auto show = controller()->uiShow();
		_sendRestriction = FrozenWriteRestriction(this, show, FrozenWriteRestrictionType::MessageField);
		...
		const auto show = controller()->uiShow();
		_sendRestriction = BoostsToLiftWriteRestriction(this, show, _peer, lifting);
	} else {
...
void HistoryWidget::updateHistoryGeometry(bool initial, bool loadedDown, const ScrollChange &change) {
	...
	if (_subsectionTabs) {
		...
		_subsectionTabs->setBoundingRect({0, subsectionTabsTop, width(), areaHeight});
	}
		...
		if (timestamp.has_value()) {
			...
			if (document && document->isVideoFile()) {
				...
				controller()->openDocument(
					document, true, {.id = item->fullId(), .showDrawButton = draw}, nullptr, timestamp);
			}
		}
	}
}
				...
				if (const auto height = view->height()) {
					...
					if (!_itemRevealAnimations.contains(item)) {
						...
						animation.animation.start(
							[=] { revealItemsCallback(); }, 0., 1., st::itemRevealDuration, anim::easeOutCirc);
						if (item->out() || _history->peer->isSelf()) {
							_list->theme()->rotateComplexGradientBackground();
						}
					}
...
void HistoryWidget::startMessageSendingAnimation(not_null<HistoryItem *> item) {
	if (_list->elementChatMode() == HistoryView::ElementChatMode::Default && width() > st::columnMaximalWidthLeft &&
		!item->media()) {
		return;
	}
...
void HistoryWidget::editDraftOptions() {
	...
	EditDraftOptions({
		...
		.done = done,
		.highlight = highlight,
		.clearOldDraft = [=] { ClearDraftReplyTo(history, MsgId(), PeerId(), replyToId); },
	});
}
...
void HistoryWidget::keyPressEvent(QKeyEvent *e) {
	...
	if (e->key() == Qt::Key_Escape) {
		...
		if (!_field->empty() || !canWriteMessage() || _editMsgId || _replyTo) {
			_scroll->keyPressEvent(e);
		} else {
...
bool HistoryWidget::replyToPreviousMessage() {
	...
	const auto fullId =
		FullMsgId(_history->peer->id,
				  ((isFieldVisible && _replyTo.messageId.msg) ? _replyTo.messageId.msg
...
void HistoryWidget::setupTranslateBar() {
	...
	_translateBar = std::make_unique<HistoryView::TranslateBar>(_topBars.get(), controller(), _history);
...
void HistoryWidget::checkPinnedBarState() {
	...
	auto pinnedRefreshed = Info::Profile::SharedMediaCountValue(_peer,
		...
		rpl::distinct_until_changed();
	auto customButtonItem = HistoryView::PinnedBarItemWithCustomButton(&session(), _pinnedTracker->shownMessageId());
	rpl::combine(rpl::duplicate(pinnedRefreshed), rpl::duplicate(customButtonItem)) |
		rpl::on_next([=](bool many, HistoryItem *item) { refreshPinnedBarButton(many, item); }, _pinnedBar->lifetime());
...
void HistoryWidget::refreshPinnedBarButton(bool many, HistoryItem *item) {
	...
	button->setAccessibleName(close ? tr::lng_cancel(tr::now) : tr::lng_settings_events_pinned(tr::now));
	button->clicks() |
		rpl::on_next(
			[=]
			{
...
void HistoryWidget::setupRequestsBar() {
	...
	_requestsBar = std::make_unique<Ui::RequestsBar>(
		_topBars.get(), HistoryView::RequestsBarContentByPeer(peer, st::historyRequestsUserpics.size, false));
	...
	_requestsBar->barClicks() |
		rpl::on_next([=] { RequestsBoxController::Start(controller(), _peer); }, _requestsBar->lifetime());
...
void HistoryWidget::createSponsoredMessageBar() {
	_sponsoredMessageBar = base::make_unique_q<Ui::SlideWrap<>>(_topBars.get(), object_ptr<Ui::RpWidget>(this));
		...
		rpl::on_next(
			[this]
			{
				...
				_sponsoredMessageBar->shownValue() | rpl::filter(!rpl::mappers::_1) |
					rpl::on_next([this] { _sponsoredMessageBar = nullptr; }, _sponsoredMessageBar->lifetime());
			},
...
bool HistoryWidget::sendExistingDocument(not_null<DocumentData *> document,
	...
	const auto checked = checkSendPayment(1, messageToSend.action.options, withPaymentApproved);
	if (!checked) {
		return false;
	}
...
void HistoryWidget::validateSubsectionTabs() {
	if (!_subsectionCheckLifetime && _history && _history->peer->isMegagroup()) {
		_subsectionCheckLifetime = _history->peer->asChannel()->flagsValue() | rpl::skip(1) |
			rpl::filter(
									   [=](Data::Flags<ChannelDataFlags>::Change change)
									   {
										   const auto mask = ChannelDataFlag::Forum | ChannelDataFlag::ForumTabs |
											   ChannelDataFlag::MonoforumAdmin;
										   return change.diff & mask;
									   }) |
			rpl::on_next([=] { validateSubsectionTabs(); });
	}
	...
	if (!_subsectionTabs) {
		_subsectionTabs = std::make_unique<HistoryView::SubsectionTabs>(controller(), this, _history);
	}
	_subsectionTabs->removeRequests() |
		rpl::on_next(
			[=]
			{
				_subsectionTabsLifetime.destroy();
	...
	_subsectionTabs->layoutRequests() |
		rpl::on_next(
			[=]
			{
...
void HistoryWidget::replyToMessage(not_null<HistoryItem *> item, FullReplyTo fields) {
	if (isJoinChannel()) {
		return;
	}
...
void HistoryWidget::processReply() {
	...
	if (!_processingReplyItem) {
		...
		if (forum->topicDeleted(topicRootId) && !(topicRootId == ForumTopic::kGeneralId && _peer->isBot())) {
			return processCancel();
	...
	if (_editMsgId) {
		if (const auto localDraft = _history->localDraft({}, {})) {
			...
			_history->setLocalDraft(std::make_unique<Data::Draft>(
				TextWithTags(), id, SuggestOptions(), MessageCursor(), Data::WebPageDraft()));
		}
	} else {
		_replyEditMsg = item;
		_replyTo = id;
		if (_replyTo) {
			if (const auto i = session().data().message(_replyTo.messageId)) {
				if (const auto media = i->media()) {
					using namespace SendMenu;
					const auto type = media->hasSpoiler() ? Action{.type = Action::Type::SpoilerOn}
...
void HistoryWidget::editMessage(not_null<HistoryItem *> item, const TextSelection &selection) {
	...
	_history->setLocalEditDraft(
		std::make_unique<Data::Draft>(editData, FullReplyTo{item->fullId()}, SuggestOptions(), cursor, previewDraft));
	applyDraft();
...
bool HistoryWidget::cancelReply(bool lastKeyboardUsed, bool keepHighlighterState) {
	bool wasReply = false;
	if (_replyTo) {
		wasReply = true;
...
void HistoryWidget::fullInfoUpdated() {
	...
	if (_history && HistoryView::SubsectionTabs::UsedFor(_history) && !_subsectionTabs) {
		validateSubsectionTabs();
	}
	if (refresh) {
		updateControlsVisibility();
...
bool HistoryWidget::updateCanSendMessage() {
	...
	const auto newCanSendMessages = restrictedOnlyReplies ? false
		: topic											  ? Data::CanSendAnyOf(topic, allWithoutPolls)
														  : Data::CanSendAnyOf(_peer, allWithoutPolls);
	const auto newCanSendTexts = restrictedOnlyReplies ? false
		: topic										   ? Data::CanSend(topic, ChatRestriction::SendOther)
...
void HistoryWidget::confirmDeleteSelected() {
	...
	if (CanCreateModerateMessagesBox(items)) {
		...
		controller()->show(Box(CreateModerateMessagesBox, items, crl::guard(this, [=] { clearSelected(); }), opt));
	} else {
...
void HistoryWidget::messageShotSelected() {
	...
	box->boxClosing() | rpl::on_next([=] { clearSelected(); }, box->lifetime());
...
void HistoryWidget::updateTopBarSelection() {
	...
	_reportMessages->setText(selectedState.count
								 ? tr::lng_report_messages_count(tr::now, lt_count, selectedState.count, tr::upper)
...
void HistoryWidget::updateReplyEditText(not_null<HistoryItem *> item) {
	...
	const auto text = [&]
	{
		...
		if (const auto todolist = media ? media->todolist() : nullptr) {
			const auto i = ranges::find(todolist->items, _replyTo.todoItemId, &TodoListItem::id);
			if (i != end(todolist->items)) {
				return i->text;
			}
		}
		return (_editMsgId || _replyTo.quote.empty()) ? item->inReplyText() : _replyTo.quote;
	}();
	_replyEditMsgText.setMarkedText(st::defaultTextStyle, text, Ui::DialogTextOptions(), context);
...
void HistoryWidget::updateForwarding() {
	_forwardPanel->update(_history,
						  _history ? _history->resolveForwardDraft(MsgId(), PeerId()) : Data::ResolvedForwardDraft());
	updateControlsVisibility();
	updateControlsGeometry();
}
			...
			if (_suggestOptions) {
				...
				if (_editMsgId) {
					...
					_replyToName.drawElided(p,
											...
											width() - replyLeft - _fieldBarCancel->width() -
				...
				_replyEditMsgText.draw(
					...
					{
						.position =
							QPoint(replyLeft, st::msgReplyPadding.top() + st::msgServiceNameFont->height + backy),
