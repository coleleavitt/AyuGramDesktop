...
void HistoryWidget::fileChosen(ChatHelpers::FileChosen &&data) {
	...
	if (const auto info = data.document->sticker()
		...
		const auto &settings = AyuSettings::getInstance();
		if (!settings.sendReadMessages && !AyuSettings::isGhostExempt(_history->peer->id.value) && settings.markReadAfterAction) {
...
void HistoryWidget::send(Api::SendOptions options) {
	...
	if (!settings.sendReadMessages && !AyuSettings::isGhostExempt(_history->peer->id.value) && settings.markReadAfterAction && lastMessage) {
		readHistory(lastMessage);
	}
