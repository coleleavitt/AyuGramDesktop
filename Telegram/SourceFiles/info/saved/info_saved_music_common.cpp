...
[[nodiscard]] Profile::MusicButtonData DocumentMusicButtonData(not_null<DocumentData *> document,
...
void SetupSavedMusic(not_null<Ui::VerticalLayout *> container,
	...
	auto musicValue = Data::SavedMusic::Supported(peer->id) ? Data::SavedMusicList(peer, nullptr, 1) |
			rpl::map([=](const Data::SavedMusicSlice &data) { return data.size() ? data[0].get() : nullptr; }) |
			rpl::type_erased
	...
	rpl::combine(std::move(musicValue), std::move(topBarColor)) |
		rpl::on_next(
			[=](HistoryItem *item, std::optional<QColor> color)
			{
				while (divider->entity()->count()) {
					delete divider->entity()->widgetAt(0);
				}
				if (item) {
					if (const auto document = item->media() ? item->media()->document() : nullptr) {
						auto musicButton = divider->entity()->add(object_ptr<Ui::SlideWrap<Profile::AyuMusicButton>>(
							...
							object_ptr<Profile::AyuMusicButton>(
								...
								color,
								[window = controller, peer] { window->showSection(Info::Saved::MakeMusic(peer)); })));

						musicButton->hide(anim::type::instant);
						musicButton->setDuration(250);
						musicButton->entity()->setAcceptBoth(true);
						musicButton->entity()->clicks() |
							rpl::filter([=](Qt::MouseButton mouseButton) { return mouseButton == Qt::RightButton; }) |
							rpl::on_next(
								[=]
								{
									...
									contextMenu->addAction(
										settings.adaptiveCoverColor ? tr::ayu_DisableColorfulCover(tr::now)
										...
										[=]
										{
											...
											const auto documentRefreshed =
												mediaRefreshed ? mediaRefreshed->document() : nullptr;
