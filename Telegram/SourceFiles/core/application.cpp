...
Application::Application()
	: QObject(), _private(std::make_unique<Private>()), _platformIntegration(Platform::Integration::Create()),
	  ...
	  _setupEmailLock(false), _autoLockTimer([=] { checkAutoLock(); }) {
	...
	settings().windowTitleContentChanges() | rpl::on_next([=] { updateWindowTitles(); }, _lifetime);
...
void Application::run() {
	...
	rpl::combine(_batterySaving->value(), settings().ignoreBatterySavingValue()) |
		rpl::on_next([=](bool saving, bool ignore) { PowerSaving::SetForceAll(saving && !ignore); }, _lifetime);
	...
	[[maybe_unused]] const auto &webviewAvailability = Core::CachedWebviewAvailability();
	...
	_domain->activeChanges() |
		rpl::on_next([=](not_null<Main::Account *> account) { showAccount(account); }, _lifetime);
...
void Application::createTray() {
	...
	_tray->hideToTrayRequests() |
		rpl::on_next([=] { enumerateWindows([&](WindowRaw w) { w->widget()->minimizeToTray(); }); }, _lifetime);
}
					...
					InvokeQueued(this,
								 [=]
								 {
									 const auto activateRequired = ranges::any_of(
										 ranges::views::all(_urlsToOpen) |
											 ranges::views::transform([](const QUrl &url) { return url.toString(); }),
...
void Application::badMtprotoConfigurationError() {
	if (settings().proxy().isEnabled() && !_badProxyDisableBox) {
		...
		_badProxyDisableBox->boxClosing() | rpl::on_next(disableCallback, _badProxyDisableBox->lifetime());
	}
}
...
void Application::startLocalStorage() {
	...
	settings().saveDelayedRequests() | rpl::on_next([=] { saveSettingsDelayed(); }, _lifetime);
}
...
void Application::forceLogOut(not_null<Main::Account *> account, const TextWithEntities &explanation) {
	...
	const auto weak = base::make_weak(account);
	connect(box.get(), &QObject::destroyed, [=] { crl::on_main(weak, [=] { account->forcedLogOut(); }); });
}
...
void Application::checkStartUrls() {
	...
	if (!cRefStartUrls().isEmpty() && _lastActivePrimaryWindow && !_lastActivePrimaryWindow->locked()) {
...
void Application::lockBySetupEmail() {
	...
	enumerateWindows([&](not_null<Window::Controller *> w) { w->setupSetupEmailLock(); });
}

void Application::unlockSetupEmail() {
	_setupEmailLock = false;
	enumerateWindows([&](not_null<Window::Controller *> w) { w->clearSetupEmailLock(); });
}
...
Window::Controller *Application::windowForShowingForum(not_null<Data::Forum *> forum) const {
	...
	const auto id =
		Window::SeparateId(tabs ? Window::SeparateType::Chat : Window::SeparateType::Forum, forum->history());
	if (const auto separate = separateWindowFor(id)) {
		return separate;
	}
	auto result = (Window::Controller *) nullptr;
...
void Application::startShortcuts() {
	...
	_domain->activeSessionChanges() |
		rpl::on_next([=](Main::Session *session) { refreshApplicationIcon(session); }, _lifetime);
