...
ApiWrap::ApiWrap(not_null<Main::Session *> session)
	: MTP::Sender(&session->account().mtp()), _session(session),
	  ...
	  _updateNotifyTimer([=] { sendNotifySettingsUpdates(); }), _statsSessionKillTimer([=] { checkStatsSessions(); }),
	  ...
	  _polls(std::make_unique<Api::Polls>(this)), _todoLists(std::make_unique<Api::TodoLists>(this)),
	...
	crl::on_main(session,
				 [=]
				 {
					 ...
					 _session->data().chatsFilters().changed() |
						 ...
						 rpl::on_next([=] { requestMoreDialogsIfNeeded(); }, _session->lifetime());
...
void ApiWrap::savePinnedOrder(not_null<Data::Forum *> forum) {
	...
	request(MTPmessages_ReorderPinnedForumTopics(MTP_flags(MTPmessages_ReorderPinnedForumTopics::Flag::f_force),
												 forum->peer()->input(),
												 MTP_vector(topics)))
...
void ApiWrap::sendMessageFail(const QString &error, not_null<PeerData *> peer, uint64 randomId, FullMsgId itemId) {
	...
	if (show && error == u"PEER_FLOOD"_q) {
		...
		const auto link = tr::link(tr::lng_cant_more_info(tr::now), session().createInternalLinkFull(u"spambot"_q));
		show->showBox(Ui::MakeInformBox(tr::lng_error_public_groups_denied(tr::now, lt_more_info, link, tr::marked)),
					  Ui::LayerOption::CloseOther);
...
void ApiWrap::markContentsRead(const base::flat_set<not_null<HistoryItem *>> &items) {
	...
	for (const auto &item : items) {
		...
		if (!settings.sendReadMessages && !passthrough &&
			!AyuSettings::isGhostExempt(item->history()->peer->id.value)) {
			continue;
		}
	...
	for (const auto &channelIds : channelMarkedIds) {
		request(
			MTPchannels_ReadMessageContents(channelIds.first->inputChannel(), MTP_vector<MTPint>(channelIds.second)))
			.send();
	}
}
...
void ApiWrap::deleteSublistHistory(not_null<ChannelData *> channel, not_null<PeerData *> sublistPeer) {
	deleteSublistHistorySend(channel, sublistPeer);
}

void ApiWrap::deleteSublistHistorySend(not_null<ChannelData *> parentChat, not_null<PeerData *> sublistPeer) {
	request(MTPmessages_DeleteSavedHistory(MTP_flags(MTPmessages_DeleteSavedHistory::Flag::f_parent_peer),
										   parentChat->input(),
										   sublistPeer->input(),
										   MTP_int(0), // max_id
										   MTP_int(0), // min_date
...
void ApiWrap::requestNotifySettings(const MTPInputNotifyPeer &peer) {
	...
	const auto peerFromInput = [&](const MTPInputPeer &inputPeer)
	{
		return inputPeer.match([&](const MTPDinputPeerSelf &) { return _session->userPeerId(); },
							   ...
							   [](const MTPDinputPeerUserFromMessage &data) { return peerFromUser(data.vuser_id()); });
	};
	const auto key =
...
void ApiWrap::updateNotifySettingsDelayed(not_null<const Data::Thread *> thread) {
	...
	if (_updateNotifyTopics.emplace(topic).second) {
		topic->destroyed() | rpl::on_next([=] { _updateNotifyTopics.remove(topic); }, _updateNotifyQueueLifetime);
...
void ApiWrap::sendNotifySettingsUpdates() {
	...
	for (const auto &topic : base::take(_updateNotifyTopics)) {
		request(
			MTPaccount_UpdateNotifySettings(MTP_inputNotifyForumTopic(topic->peer()->input(), MTP_int(topic->rootId())),
...
void ApiWrap::saveDraftsToCloud() {
	for (auto i = begin(_draftsSaveRequestIds); i != end(_draftsSaveRequestIds);) {
		...
		if (!_session->supportMode()) {
			cloudDraft = history->createCloudDraft(topicRootId, monoforumPeerId, localDraft);
		} else if (!cloudDraft) {
			cloudDraft = history->createCloudDraft(topicRootId, monoforumPeerId, nullptr);
		}
		...
		if (cloudDraft->reply.messageId || cloudDraft->reply.topicRootId || cloudDraft->reply.monoforumPeerId) {
				...
				.done(
					[=](const MTPBool &result, const MTP::Response &response)
					{
						...
						history->finishSavingCloudDraft(
							topicRootId, monoforumPeerId, Api::UnixtimeFromMsgId(response.outerMsgId));
						const auto cloudDraft = history->cloudDraft(topicRootId, monoforumPeerId);
						if (cloudDraft) {
		...
		[&](Data::FileOriginMessage data)
		{
			if (const auto item = _session->data().message(data)) {
				...
				const auto storyId = mediaStory
					? mediaStory
					: FullStoryId{(IsStoryMsgId(item->id) ? item->history()->peer->id : PeerId()),
								  (IsStoryMsgId(item->id) ? StoryIdFromMsgId(item->id) : StoryId())};
				if (storyId) {
					...
					if (user && document) {
						request(MTPusers_GetSavedMusicByID(user->inputUser(),
					...
					request(
						MTPchannels_GetMessages(channel->inputChannel(),
												MTP_vector<MTPInputMessage>(1, MTP_inputMessageID(MTP_int(item->id)))));
				} else {
					request(
...
void ApiWrap::resolveJumpToDate(Dialogs::Key chat, const QDate &date, Fn<void(not_null<PeerData *>, MsgId)> callback) {
	if (const auto peer = chat.peer()) {
		...
		const auto monoforumPeerId = sublist ? sublist->sublistPeer()->id : PeerId();
		resolveJumpToHistoryDate(peer, rootId, monoforumPeerId, date, std::move(callback));
	}
}

template<typename Callback>
void ApiWrap::requestMessageAfterDate(
	not_null<PeerData *> peer, MsgId topicRootId, PeerId monoforumPeerId, const QDate &date, Callback &&callback) {
	...
	if (topicRootId) {
		...
		send(MTPmessages_GetSavedHistory(MTP_flags(MTPmessages_GetSavedHistory::Flag::f_parent_peer),
										 peer->input(),
...
void ApiWrap::resolveJumpToHistoryDate(not_null<PeerData *> peer,
	...
	if (const auto channel = peer->migrateTo()) {
		return resolveJumpToHistoryDate(channel, topicRootId, monoforumPeerId, date, std::move(callback));
	}
	const auto jumpToDateInPeer = [=]
	{
		requestMessageAfterDate(
			peer, topicRootId, monoforumPeerId, date, [=](MsgId itemId) { callback(peer, itemId); });
	};
	const auto migrated = (topicRootId || monoforumPeerId) ? nullptr : peer->migrateFrom();
	if (migrated) {
		requestMessageAfterDate(migrated,
								MsgId(),
								PeerId(),
								date,
								[=](MsgId itemId)
...
void ApiWrap::requestSharedMedia(not_null<PeerData *> peer,
								 MsgId topicRootId,
								 PeerId monoforumPeerId,
	...
	const auto prepared =
		Api::PrepareSearchRequest(peer, topicRootId, monoforumPeerId, type, QString(), messageId, slice);
	if (!prepared) {
		return;
	}
							  ...
							  return request(std::move(*prepared))
								  .done(
									  [=](const Api::SearchRequestResult &result)
									  {
										  ...
										  sharedMediaDone(peer, topicRootId, monoforumPeerId, type, std::move(parsed));
										  finish();
									  })
								  .fail(
									  [=]
...
void ApiWrap::sharedMediaDone(not_null<PeerData *> peer,
	...
	_session->storage().add(Storage::SharedMediaAddSlice(peer->id,
														 topicRootId,
														 monoforumPeerId,
														 type,
...
void ApiWrap::sendAction(const SendAction &action) {
	if (!action.options.scheduled && !action.options.shortcutId && !action.replaceMediaOf) {
		...
		const auto sublist = monoforumPeerId ? action.history->peer->monoforumSublistFor(monoforumPeerId) : nullptr;
		if (topic) {
			topic->readTillEnd();
		} else if (sublist) {
			sublist->readTillEnd();
...
void ApiWrap::forwardMessages(Data::ResolvedForwardDraft &&draft, SendAction action, FnMut<void()> &&successCallback) {
	...
	if (fullAyuForward) {
		crl::async([=] { AyuForward::forwardMessages(_session, action, false, draft); });
		return;
	}

	const auto ayuIntelligentForwardNeeded =
		AyuForward::isAyuForwardNeeded(draft.items) || AyuSettings::getInstance().alwaysUseIntelligentForward;
	if (ayuIntelligentForwardNeeded) {
		crl::async([=] { AyuForward::intelligentForward(_session, action, draft); });
		return;
	}
	...
	const auto monoforumPeerId = action.replyTo.monoforumPeerId;
	const auto monoforumPeer = monoforumPeerId ? session().data().peer(monoforumPeerId).get() : nullptr;
	...
	const auto sendAccumulated = [&]
	{
		...
		auto buildMessage = [=](not_null<History *> history, FullReplyTo replyTo) -> Data::Histories::PreparedMessage
		{
			const auto kGeneralId = Data::ForumTopic::kGeneralId;
			...
			return MTPmessages_ForwardMessages(MTP_flags(flags),
											   forwardFrom->input(),
		...
		histories.sendPreparedMessage(
			...
			[=](const MTPUpdates &result, const MTP::Response &)
			{
				...
				const auto &settings = AyuSettings::getInstance();
				if (!settings.sendReadMessages && !AyuSettings::isGhostExempt(history->peer->id.value) &&
					settings.markReadAfterAction && history->lastMessage()) {
					readHistory(history->lastMessage());
				}

				if (peer->isSelf() && _session->premium()) {
					ProcessRecentSelfForwards(_session, result, peer->id, forwardFrom->id);
				}
			},
...
void ApiWrap::editMedia(Ui::PreparedList &&list, SendMediaType type, TextWithTags &&caption, const SendAction &action) {
	...
	const auto existing =
		to.replaceMediaOf ? session().data().message(action.history->peer, to.replaceMediaOf) : nullptr;
	if (existing && existing->computeSuggestionActions() == SuggestionActions::AcceptAndDecline) {
	...
	const auto forceFile = (type == SendMediaType::File) && (file.type == Ui::PreparedFile::Type::Video);
	_fileLoader->addTask(std::make_unique<FileLoadTask>(FileLoadTask::Args{
		.session = &session(),
		.filepath = file.path,
		.content = file.content,
		.information = std::move(file.information),
		.videoCover = (file.videoCover ? std::make_unique<FileLoadTask>(FileLoadTask::Args{
											 .session = &session(),
...
void ApiWrap::sendFile(const QByteArray &fileContent, SendMediaType type, const SendAction &action) {
	...
	_fileLoader->addTask(std::make_unique<FileLoadTask>(FileLoadTask::Args{.session = &session(),
																		   .filepath = QString(),
																		   .content = fileContent,
																		   .information = nullptr,
																		   .videoCover = nullptr,
...
void ApiWrap::sendMessage(MessageToSend &&message, std::optional<MsgId> localMessageId) {
	...
	const bool canSendTexts = topic ? Data::CanSendTexts(topic) : Data::CanSendTexts(peer);
	...
	while (TextUtilities::CutPart(sending, left, MaxMessageSize) || (isFirst && exactWebPage)) {
		...
		auto newId = FullMsgId(peer->id,
							   localMessageId ? std::exchange(localMessageId, std::nullopt).value()
		...
		const auto sentEntities =
			Api::EntitiesToMTP(_session, sendingNormalized.entities, Api::ConvertOption::SkipLocal);
		...
		const auto done = [=](const MTPUpdates &result, const MTP::Response &response)
		{
			if (clearCloudDraft) {
				history->finishSavingCloudDraft(
					draftTopicRootId, draftMonoforumPeerId, Api::UnixtimeFromMsgId(response.outerMsgId));
			}
		};
...
void ApiWrap::sendInlineResult(not_null<UserData *> bot,
	...
	histories.sendPreparedMessage(
		...
		{
			history->finishSavingCloudDraft(topicRootId, monoforumPeerId, Api::UnixtimeFromMsgId(response.outerMsgId));
			if (done) {
				done(true);
			}
		},
...
void ApiWrap::sendMediaWithRandomId(not_null<HistoryItem *> item,
	...
	auto sentEntities = Api::EntitiesToMTP(_session, captionNormalized.entities, Api::ConvertOption::SkipLocal);
	...
	const auto flags = Flag(0) | (replyTo ? Flag::f_reply_to : Flag(0)) |
		...
		(options.sendAs ? Flag::f_send_as : Flag(0)) | (options.shortcutId ? Flag::f_quick_reply_shortcut : Flag(0)) |
		(options.effectId ? Flag::f_effect : Flag(0)) | (options.suggest ? Flag::f_suggested_post : Flag(0)) |
...
void ApiWrap::sendMultiPaidMedia(not_null<HistoryItem *> item, not_null<SendingAlbum *> album, Fn<void(bool)> done) {
	...
	const auto flags = Flag(0) | (replyTo ? Flag::f_reply_to : Flag(0)) |
		...
		(!sentEntities.v.isEmpty() ? Flag::f_entities : Flag(0)) |
		(options.scheduled ? Flag::f_schedule_date : Flag(0)) |
		(options.scheduleRepeatPeriod ? Flag::f_schedule_repeat_period : Flag(0)) |
		(options.sendAs ? Flag::f_send_as : Flag(0)) | (options.shortcutId ? Flag::f_quick_reply_shortcut : Flag(0)) |
		(options.effectId ? Flag::f_effect : Flag(0)) | (options.suggest ? Flag::f_suggested_post : Flag(0)) |
		(options.invertCaption ? Flag::f_invert_media : Flag(0)) | (starsPaid ? Flag::f_allow_paid_stars : Flag(0));
...
void ApiWrap::sendAlbumIfReady(not_null<SendingAlbum *> album) {
	...
	const auto flags = Flag(0) | (replyTo ? Flag::f_reply_to : Flag(0)) |
		...
		//	: Flag(0))
		| (sendAs ? Flag::f_send_as : Flag(0)) | (album->options.shortcutId ? Flag::f_quick_reply_shortcut : Flag(0)) |
	...
	histories.sendPreparedMessage(
		...
		Data::Histories::PrepareMessage<MTPmessages_SendMultiMedia>(
			...
			MTP_long(starsPaid)),
		[=](const MTPUpdates &result, const MTP::Response &response) { _sendingAlbums.remove(groupId); },
		[=](const MTP::Error &error, const MTP::Response &response)
		{
			if (const auto album = _sendingAlbums.take(groupId)) {
				for (const auto &item : (*album)->items) {
