...
ChatPaintContext::quoteCache(const std::shared_ptr<ColorCollectible> &colorCollectible, uint8 colorIndex) const {
	return outbg		   ? messageStyle()->quoteCache[colorCollectible ? 2 : st->colorPatternIndex(colorIndex)].get()
		: colorCollectible ? st->collectibleQuoteCache(selected(), colorCollectible).get()
...
ChatStyle::ColoredPalette::ColoredPalette(const ColoredPalette &other) : linkFg(other.linkFg), data(other.data) {
	if (linkFg) {
...
ChatStyle::ColoredPalette &ChatStyle::ColoredPalette::operator=(const ColoredPalette &other) {
	linkFg = other.linkFg;
	data = other.data;
	if (linkFg) {
		data.linkFg = linkFg->color();
...
ChatStyle::ChatStyle(rpl::producer<ColorIndicesCompressed> colorIndices) {
	if (colorIndices) {
		_colorIndicesLifetime = std::move(colorIndices) |
			rpl::on_next([=](ColorIndicesCompressed &&indices) { _colorIndices = std::move(indices); });
	}
	...
	make(&MessageStyle::historyCallGroupIcon,
...
void ChatStyle::applyCustomPalette(const style::palette *palette) {
	...
	if (palette) {
		...
		style::PaletteChanged() |
			rpl::on_next([=] { assignPalette(style::main_palette::get()); }, _defaultPaletteChangeLifetime);
	}
}
...
int ChatStyle::collectiblePatternIndex(const std::shared_ptr<ColorCollectible> &collectible) const {
	const auto &strip = (_dark && !collectible->darkStrip.empty()) ? collectible->darkStrip : collectible->strip;
...
QColor ChatStyle::collectibleNameColor(const std::shared_ptr<ColorCollectible> &collectible) const {
	return (_dark && collectible->darkAccentColor.alpha() > 0) ? collectible->darkAccentColor
...
const style::TextPalette &
ChatStyle::collectibleTextPalette(bool selected, const std::shared_ptr<ColorCollectible> &collectible) const {
...
not_null<BackgroundEmojiData *>
ChatStyle::backgroundEmojiData(uint64 emojiId, const std::shared_ptr<ColorCollectible> &collectible) const {
...
not_null<Text::QuotePaintCache *>
ChatStyle::collectibleQuoteCache(bool selected, const std::shared_ptr<ColorCollectible> &collectible) const {
	auto &entry = resolveCollectibleCaches(collectible);
	return collectibleCache(selected ? entry.quoteSelected : entry.quote, collectible);
}

not_null<Text::QuotePaintCache *>
ChatStyle::collectibleReplyCache(bool selected, const std::shared_ptr<ColorCollectible> &collectible) const {
	auto &entry = resolveCollectibleCaches(collectible);
	return collectibleCache(selected ? entry.replySelected : entry.reply, collectible);
}
...
not_null<Text::QuotePaintCache *>
ChatStyle::collectibleCache(std::unique_ptr<Text::QuotePaintCache> &cache,
	...
	EnsureBlockquoteCache(cache,
						  [&]
						  {
							  ...
							  const auto &strip = (_dark && !collectible->darkStrip.empty()) ? collectible->darkStrip
																							 : collectible->strip;
...
ChatStyle::CollectibleColors &
ChatStyle::resolveCollectibleCaches(const std::shared_ptr<ColorCollectible> &collectible) const {
...
QColor FromNameFg(not_null<const ChatStyle *> st,
				  bool selected,
				  uint8 colorIndex,
				  const std::shared_ptr<Ui::ColorCollectible> &colorCollectible) {
	return !colorCollectible											  ? st->coloredValues(selected, colorIndex).name
		: (st->dark() && (colorCollectible->darkAccentColor.alpha() > 0)) ? colorCollectible->darkAccentColor
