...
void TranslateBox(not_null<Ui::GenericBox *> box,
	...
	const auto reqMsgId = request->msgId;
	const auto flags = reqMsgId ? (Flag::f_peer | Flag::f_id) : !request->text.text.isEmpty() ? Flag::f_text : Flag(0);
				...
				[=](LanguageId to, Fn<void(TranslateBoxContentResult)> done)
			{
				...
				Ayu::Translator::TranslateManager::currentInstance()
					->request(&peer->session(),
							  MTP_flags(flags),
							  reqMsgId ? peer->input() : MTP_inputPeerEmpty(),
							  (reqMsgId ? MTP_vector<MTPint>(1, MTP_int(reqMsgId)) : MTPVector<MTPint>()),
							  (reqMsgId ? MTPVector<MTPTextWithEntities>()
										: MTP_vector<MTPTextWithEntities>(
											  1,
											  MTP_textWithEntities(MTP_string(request->text.text),
																   Api::EntitiesToMTP(&peer->session(),
					...
					.done(
						[=](const MTPmessages_TranslatedText &result)
						{
							...
							if (list.isEmpty()) {
									...
									.text = Api::ParseTextWithEntities(&peer->session(), list.front()),
									.error = UiError::None,
								});
							}
						})
					.fail(
						[done](const MTP::Error &error)
						{
							done(TranslateBoxContentResult{
								.error = UiError::Unknown,
							});
