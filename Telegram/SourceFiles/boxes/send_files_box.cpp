...
SendFilesBox::Block::Block(not_null<QWidget *> parent,
	...
	if (_isAlbum) {
		...
		const auto media = way.sendImagesAsPhotos()
			? Ui::SingleMediaPreview::Create(parent,
											 st,
											 gifPaused,
											 first,
...
SendFilesBox::SendFilesBox(QWidget *, SendFilesBoxDescriptor &&descriptor)
	: _show(std::move(descriptor.show)),
	  ...
	  _caption(this, _st.files.caption, Ui::InputField::Mode::MultiLine, tr::lng_photo_caption()),
...
void SendFilesBox::pushBlock(int from, int till) {
	...
	_blocks.emplace_back(_inner.data(),
						 ...
						 [=](const Ui::PreparedFile &file, Ui::AttachActionType type)
						 {
							 return (type == Ui::AttachActionType::ToggleSpoiler) ? !hasPrice()
								 ...
								 ? (file.isVideoFile() && (_toPeer->isBroadcast() || _toPeer->isSelf()))
	...
	const auto showContextMenu = [=](int fileIndex, QPoint globalPosition)
	{
		...
		if (canToggleSpoiler) {
			...
			state->menu->addAction(
				spoilered ? tr::lng_context_disable_spoiler(tr::now) : tr::lng_context_spoiler_effect(tr::now),
				[=]
				{
					applyBlockChanges();
					refreshAllAfterChanges(from,
	...
	block.itemClearCoverRequest() | rpl::on_next([=](int index) { clearCover(index); }, widget->lifetime());
