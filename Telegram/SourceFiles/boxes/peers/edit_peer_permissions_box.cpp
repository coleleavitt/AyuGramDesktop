...
[[nodiscard]] auto Dependencies(PowerSaving::Flags) -> std::vector<std::pair<PowerSaving::Flag, PowerSaving::Flag>> {
	return {};
}
...
[[nodiscard]] auto NestedRestrictionLabelsList(Data::RestrictionsSetOptions options)
	...
	auto media = std::vector<RestrictionLabel>{
		...
		{Flag::SendStickers | Flag::SendGifs | Flag::SendGames | Flag::SendInline,
	...
	auto second = std::vector<RestrictionLabel>{
		...
		{Flag::EditRank,
		 (options.isUserSpecific ? tr::lng_rights_group_edit_rank_single : tr::lng_rights_group_edit_rank)(tr::now)},
		{Flag::ChangeInfo, tr::lng_rights_group_info(tr::now)},
	};
	if (!options.isForum) {
		second.erase(ranges::remove(second, Flag::CreateTopics | Flag(), &RestrictionLabel::flags), end(second));
	}
	return {
...
[[nodiscard]] auto NestedAdminRightLabels(Data::AdminRightsSetOptions options)
	...
	if (options.isGroup) {
		auto first = std::vector<AdminRightLabel>{
			...
			{Flag::InviteByLinkOrAdd,
			 options.anyoneCanAddMembers ? tr::lng_rights_group_invite_link(tr::now)
		...
		if (!options.isForum) {
			first.erase(ranges::remove(first, Flag::ManageTopics | Flag(), &AdminRightLabel::flags), end(first));
		}
		return {
			{std::nullopt, std::move(first)},
...
[[nodiscard]] int BoostsUnrestrictByIndex(int index) { return index + 1; }

template<typename CheckboxesMap, typename DependenciesMap>
void ApplyDependencies(const CheckboxesMap &checkboxes,
					   const DependenciesMap &dependencies,
					   Ui::AbstractCheckView *changed) {
	const auto checkAndApply = [&](auto &&current, auto dependency, bool isChecked)
	{
	...
	const auto applySomeDependency = [&]
	{
		...
		for (auto &&entry : checkboxes) {
			...
			for (auto &&dependency : dependencies) {
				const auto check = isChecked ? dependency.first : dependency.second;
...
auto Dependencies(ChatRestrictions) -> std::vector<std::pair<ChatRestriction, ChatRestriction>> {
	using Flag = ChatRestriction;
...
ChatRestrictions NegateRestrictions(ChatRestrictions value) {
	...
	return (~value) &
		(Flag(0)
		 ...
		 //| Flag::ViewMessages
		 | Flag::ChangeInfo | Flag::EmbedLinks | Flag::AddParticipants | Flag::CreateTopics | Flag::PinMessages |
		 Flag::SendGames | Flag::SendGifs | Flag::SendInline | Flag::SendPolls | Flag::SendStickers | Flag::SendPhotos |
		 Flag::SendVideos | Flag::SendVideoMessages | Flag::SendMusic | Flag::SendVoiceMessages | Flag::SendFiles |
		 Flag::SendOther | Flag::EditRank);
}

auto Dependencies(ChatAdminRights) -> std::vector<std::pair<ChatAdminRight, ChatAdminRight>> { return {}; }

auto ToPositiveNumberString() {
	return rpl::map([](int count) { return count ? QString::number(count) : QString(); });
}
...
ChatRestrictions DisabledByAdminRights(not_null<PeerData *> peer) {
	...
	return Flag(0) | ((adminRights & Admin::ManageTopics) ? Flag(0) : Flag::CreateTopics) |
		((adminRights & Admin::PinMessages) ? Flag(0) : Flag::PinMessages) |
		((adminRights & Admin::InviteByLinkOrAdd) ? Flag(0) : Flag::AddParticipants) |
		((adminRights & Admin::ChangeInfo) ? Flag(0) : Flag::ChangeInfo);
}
...
not_null<Ui::RpWidget *> AddInnerToggle(not_null<Ui::VerticalLayout *> container,
	...
	const auto button = container->add(object_ptr<Ui::SettingsButton>(container, nullptr, st));
	if (icon) {
	...
	for (const auto &innerCheck : state->innerChecks) {
		innerCheck->checkedChanges() | rpl::to_empty | rpl::start_to_stream(state->anyChanges, button->lifetime());
	}
	...
	state->anyChanges.events_starting_with(rpl::empty_value()) | rpl::map(countChecked) |
		rpl::on_next([=](int count) { checkView->setChecked(count > 0, anim::type::normal); },
					 toggleButton->lifetime());
	...
	const auto label = Ui::CreateChild<Ui::FlatLabel>(
		button,
		rpl::combine(std::move(buttonLabel),
					 state->anyChanges.events_starting_with(rpl::empty_value()) | rpl::map(countChecked)) |
			rpl::map(
				[=](const QString &t, int checked)
				{
					auto count = tr::bold("  " + QString::number(checked) + '/' + QString::number(totalInnerChecks));
	...
	button->sizeValue() |
		rpl::on_next(
			[=, &st](const QSize &s)
			{
				...
				arrow->moveToLeft(std::min(labelLeft + label->textMaxWidth(), labelRight - arrow->width()),
...
[[nodiscard]] EditFlagsControl<Flags>
CreateEditFlags(not_null<Ui::VerticalLayout *> container, Flags checked, EditFlagsDescriptor<Flags> &&descriptor) {
	struct State final
	{
	...
	if (descriptor.forceDisabledMessage) {
		state->forceDisabledMessage = std::move(descriptor.forceDisabledMessage);
		state->forceDisabled =
			state->forceDisabledMessage.value() | rpl::map([=](const QString &message) { return !message.isEmpty(); });
					...
					if (disabled) {
						...
						for (const auto &[flags, checkView] : state->checkViews) {
							if (const auto i = state->realCheckedValues.find(flags);
								...
								checkView->setChecked(i->second, anim::type::normal);
							}
						}
					}
				},
	...
	const auto addCheckbox =
		[&](not_null<Ui::VerticalLayout *> verticalLayout, bool isInner, const EditFlagsLabel<Flags> &entry)
	{
		const auto flags = entry.flags;
		const auto lockedIt =
			ranges::find_if(descriptor.disabledMessages, [&](const auto &pair) { return (pair.first & flags) != 0; });
		const auto locked =
			(lockedIt != end(descriptor.disabledMessages)) ? std::make_optional(lockedIt->second) : std::nullopt;
		...
		const auto checkView = [&]() -> not_null<Ui::AbstractCheckView *>
		{
			if (isInner) {
				const auto checkbox = verticalLayout->add(
					object_ptr<Ui::Checkbox>(verticalLayout, entry.label, toggled, st::settingsCheckbox), st.padding);
				const auto button = Ui::CreateChild<Ui::RippleButton>(verticalLayout.get(), st::defaultRippleAnimation);
				button->stackUnder(checkbox);
				rpl::combine(verticalLayout->widthValue(), checkbox->geometryValue()) |
					rpl::on_next([=](int w, const QRect &r) { button->setGeometry(0, r.y(), w, r.height()); },
								 button->lifetime());
				...
				const auto button =
					Settings::AddButtonWithIcon(verticalLayout, rpl::single(entry.label), st, {entry.icon});
				const auto toggle = Ui::CreateChild<Ui::RpWidget>(button.get());

				// Looks like a bug in Clang, fails to compile with 'auto&' below.
				rpl::lifetime &lifetime = toggle->lifetime();

				const auto checkView =
					lifetime.make_state<Ui::ToggleView>(st.toggle, toggled, [=] { toggle->update(); });
				...
				button->sizeValue() |
					rpl::on_next([=](const QSize &s)
								 { toggle->moveToRight(st.toggleSkip, (s.height() - toggle->height()) / 2); },
								 toggle->lifetime());
				button->setClickedCallback([=] { checkView->setChecked(!checkView->checked(), anim::type::normal); });
				checkView->setLocked(locked.has_value());
				return checkView;
	...
	for (const auto &nestedWithLabel : descriptor.labels) {
		...
		if (wrap) {
			...
			const auto toggle = AddInnerToggle(container,
											   st,
											   innerChecks,
											   raw,
			...
			container->widthValue() | rpl::on_next([=](int w) { raw->resizeToWidth(w); }, raw->lifetime());
		}
	}
...
void AddSlowmodeLabels(not_null<Ui::VerticalLayout *> container) {
	const auto labels =
		container->add(object_ptr<Ui::FixedHeightWidget>(container, st::normalFont->height), st::slowmodeLabelsMargin);
	for (auto i = 0; i != kSlowmodeValues; ++i) {
		const auto seconds = SlowmodeDelayByIndex(i);
		const auto label = Ui::CreateChild<Ui::LabelSimple>(
			labels,
			st::slowmodeLabel,
			(!seconds				? tr::lng_rights_slowmode_off(tr::now)
				 : (seconds < 60)	? tr::lng_seconds_tiny(tr::now, lt_count, seconds)
				 : (seconds < 3600) ? tr::lng_minutes_tiny(tr::now, lt_count, seconds / 60)
									: tr::lng_hours_tiny(tr::now, lt_count, seconds / 3600)));
		rpl::combine(labels->widthValue(), label->widthValue()) |
			rpl::on_next(
				[=](int outer, int inner)
				{
					const auto skip = st::localStorageLimitMargin;
					const auto size = st::localStorageLimitSlider.seekSize;
					const auto available = outer - skip.left() - skip.right() - size.width();
					const auto shift = (i == 0)		 ? -(size.width() / 2)
						: (i + 1 == kSlowmodeValues) ? (size.width() - (size.width() / 2) - inner)
													 : (-inner / 2);
					const auto left =
						skip.left() + (size.width() / 2) + (i * available) / (kSlowmodeValues - 1) + shift;
...
rpl::producer<int> AddSlowmodeSlider(not_null<Ui::VerticalLayout *> container, not_null<PeerData *> peer) {
	...
	const auto secondsCount = lifetime.make_state<rpl::variable<int>>(channel ? channel->slowmodeSeconds() : 0);

	container->add(object_ptr<Ui::FlatLabel>(container, tr::lng_rights_slowmode_header(), st::rightsHeaderLabel),
				   st::rightsHeaderMargin);

	AddSlowmodeLabels(container);

	const auto slider = container->add(object_ptr<Ui::MediaSlider>(container, st::localStorageLimitSlider),
									   st::localStorageLimitMargin);
	slider->resize(st::localStorageLimitSlider.seekSize);
	slider->setPseudoDiscrete(kSlowmodeValues,
							  SlowmodeDelayByIndex,
							  secondsCount->current(),
							  [=](int seconds) { (*secondsCount) = seconds; });

	auto hasSlowMode = secondsCount->value() | rpl::map(_1 != 0) | rpl::distinct_until_changed();

	auto useSeconds = secondsCount->value() | rpl::map(_1 < 60) | rpl::distinct_until_changed();

	auto interval =
		rpl::combine(std::move(useSeconds),
					 tr::lng_rights_slowmode_interval_seconds(lt_count, secondsCount->value() | tr::to_count()),
					 tr::lng_rights_slowmode_interval_minutes(lt_count, secondsCount->value() | rpl::map(_1 / 60.))) |
		rpl::map([](bool use, const QString &seconds, const QString &minutes) { return use ? seconds : minutes; });

	auto aboutText = rpl::combine(std::move(hasSlowMode),
								  tr::lng_rights_slowmode_about(),
								  tr::lng_rights_slowmode_about_interval(lt_interval, std::move(interval))) |
		rpl::map([](bool has, const QString &about, const QString &aboutInterval)
				 { return has ? aboutInterval : about; });

	container->add(
		object_ptr<Ui::DividerLabel>(container,
									 object_ptr<Ui::FlatLabel>(container, std::move(aboutText), st::boxDividerLabel),
...
void AddBoostsUnrestrictLabels(not_null<Ui::VerticalLayout *> container) {
	...
	for (auto i = 0; i != kBoostsUnrestrictValues; ++i) {
		const auto label = Ui::CreateChild<Ui::FlatLabel>(labels, st::boostsUnrestrictLabel);
		label->setMarkedText(TextWithEntities(i ? many : one).append(QString::number(i + 1)));
		rpl::combine(labels->widthValue(), label->widthValue()) |
			rpl::on_next(
				[=](int outer, int inner)
				{
					...
					const auto available = outer - skip.left() - skip.right() - size.width();
					const auto shift = (i == 0)				 ? -(size.width() / 2)
						: (i + 1 == kBoostsUnrestrictValues) ? (size.width() - (size.width() / 2) - inner)
															 : (-inner / 2);
					const auto left =
						skip.left() + (size.width() / 2) + (i * available) / (kBoostsUnrestrictValues - 1) + shift;
					label->moveToLeft(left, 0, outer);
				},
				label->lifetime());
	}
...
rpl::producer<int> AddBoostsUnrestrictSlider(not_null<Ui::VerticalLayout *> container, not_null<PeerData *> peer) {
	using namespace rpl::mappers;
	...
	const auto channel = peer->asChannel();
	auto &lifetime = container->lifetime();
	const auto boostsUnrestrict = lifetime.make_state<rpl::variable<int>>(channel ? channel->boostsUnrestrict() : 0);

	Ui::AddSkip(container);

	auto enabled = boostsUnrestrict->value() | rpl::map(_1 > 0);
	container
			->add(object_ptr<Ui::SettingsButton>(
				container, tr::lng_rights_boosts_no_restrict(), st::defaultSettingsButton))
			->toggleOn(rpl::duplicate(enabled))
			->toggledValue() |
		rpl::on_next(
			[=](bool toggled)
			{
	...
	const auto slider =
		inner->add(object_ptr<Ui::MediaSlider>(inner, st::localStorageLimitSlider), st::localStorageLimitMargin);
	slider->resize(st::localStorageLimitSlider.seekSize);
	slider->setPseudoDiscrete(kBoostsUnrestrictValues,
							  BoostsUnrestrictByIndex,
							  boostsUnrestrict->current(),
							  [=](int boosts) { (*boostsUnrestrict) = boosts; });

	inner->add(object_ptr<Ui::DividerLabel>(
				   inner,
				   object_ptr<Ui::FlatLabel>(inner,
											 rpl::conditional(boostsUnrestrict->value() | rpl::map(_1 > 0),
...
rpl::producer<int> AddBoostsUnrestrictWrapped(not_null<Ui::VerticalLayout *> container,
	...
	const auto divider = inner->add(object_ptr<Ui::SlideWrap<Ui::BoxContentDivider>>(
		inner, object_ptr<Ui::BoxContentDivider>(inner), QMargins{0, skip, 0, skip}));
...
void AddSuggestGigagroup(not_null<Ui::VerticalLayout *> container, Fn<void()> callback) {
	container->add(object_ptr<Ui::FlatLabel>(container, tr::lng_rights_gigagroup_title(), st::rightsHeaderLabel),
				   st::rightsHeaderMargin);
	container->add(EditPeerInfoBox::CreateButton(container,
	...
	container->add(object_ptr<Ui::DividerLabel>(
					   container,
					   object_ptr<Ui::FlatLabel>(container, tr::lng_rights_gigagroup_about(), st::boxDividerLabel),
					   st::proxyAboutPadding),
				   style::margins(0, st::infoProfileSkip, 0, st::infoProfileSkip));
}
...
void AddBannedButtons(not_null<Ui::VerticalLayout *> container,
	...
	container->add(EditPeerInfoBox::CreateButton(
		...
		(channel ? Info::Profile::RestrictedCountValue(channel) : rpl::single(0)) | ToPositiveNumberString(),
		[=] { ParticipantsBoxController::Start(navigation, peer, ParticipantsBoxController::Role::Restricted); },
		st::manageGroupTopicsButton,
	...
	if (channel) {
		container->add(EditPeerInfoBox::CreateButton(
			...
			Info::Profile::KickedCountValue(channel) | ToPositiveNumberString(),
			[=] { ParticipantsBoxController::Start(navigation, peer, ParticipantsBoxController::Role::Kicked); },
			st::manageGroupTopicsButton,
			{&st::menuIconRemove}));
	}
}
...
void ShowEditPeerPermissionsBox(not_null<Ui::GenericBox *> box,
								not_null<Window::SessionNavigation *> navigation,
	...
	const auto restrictions = FixDependentRestrictions(
		...
		{
			if (const auto chat = peer->asChat()) {
				return chat->defaultRestrictions() | disabledByAdminRights;
			} else if (const auto channel = peer->asChannel()) {
				return channel->defaultRestrictions() |
					(channel->isPublic() ? (Flag::ChangeInfo | Flag::PinMessages) : Flags(0)) | disabledByAdminRights;
			}
	...
	const auto disabledMessages = [&]
	{
		...
		result.emplace(disabledByAdminRights, tr::lng_rights_permission_cant_edit(tr::now));
		if (const auto channel = peer->asChannel()) {
			if (channel->isPublic()) {
				result.emplace(Flag::ChangeInfo | Flag::PinMessages, tr::lng_rights_permission_unavailable(tr::now));
			} else if (channel->isMegagroup() && channel->discussionLink()) {
				result.emplace(Flag::ChangeInfo | Flag::PinMessages, tr::lng_rights_permission_in_discuss(tr::now));
			}
		}
		return result;
	}();

	Ui::AddSubsectionTitle(inner, tr::lng_rights_default_restrictions_header());
	auto [checkboxes, getRestrictions, changes, highlightWidget] =
		CreateEditRestrictions(inner, restrictions, disabledMessages, {.isForum = peer->isForum()});
	...
	if (available) {
		Ui::AddSkip(inner);
		const auto starsPerMessage = peer->isChannel() ? peer->asChannel()->commonStarsPerMessage() : 0;
		charging =
			inner->add(object_ptr<Ui::SettingsButton>(inner, tr::lng_rights_charge_stars(), st::settingsButtonNoIcon));
		charging->toggleOn(rpl::single(starsPerMessage > 0));
		Ui::AddSkip(inner);
		Ui::AddDividerText(inner, tr::lng_rights_charge_stars_about());

		const auto chargeWrap =
		...
		state->starsPerMessage = SetupChargeSlider(
			chargeInner, peer, (starsPerMessage > 0) ? starsPerMessage : std::optional<int>(), kDefaultChargeStars);
	}

	static constexpr auto kSendRestrictions = Flag::EmbedLinks | Flag::SendGames | Flag::SendGifs | Flag::SendInline |
		Flag::SendPolls | Flag::SendStickers | Flag::SendPhotos | Flag::SendVideos | Flag::SendVideoMessages |
		Flag::SendMusic | Flag::SendVoiceMessages | Flag::SendFiles | Flag::SendOther;
	state->hasSendRestrictions =
		((restrictions & kSendRestrictions) != 0) || (peer->isChannel() && peer->asChannel()->slowmodeSeconds() > 0);
	state->boostsUnrestrict = AddBoostsUnrestrictWrapped(inner, peer, state->hasSendRestrictions.value());
	state->slowmodeSeconds = AddSlowmodeSlider(inner, peer);
	state->hasSendRestrictions =
		rpl::combine(rpl::single(restrictions) | rpl::then(std::move(changes)), state->slowmodeSeconds.value()) |
	...
	if (const auto channel = peer->asChannel()) {
		...
		const auto threshold = -kThresholdOffset + channel->session().serverConfig().megagroupSizeMax;
		if (channel->amCreator() && channel->membersCount() >= threshold) {
			AddSuggestGigagroup(inner, AboutGigagroupCallback(peer->asChannel(), navigation->parentController()));
		}
	}
	...
	box->addButton(tr::lng_settings_save(),
				   [=, rights = getRestrictions]
				   {
					   const auto restrictions = rights();
					   const auto slowmodeSeconds = state->slowmodeSeconds.current();
					   const auto hasRestrictions = (slowmodeSeconds > 0) || ((restrictions & kSendRestrictions) != 0);
					   const auto boostsUnrestrict = hasRestrictions ? state->boostsUnrestrict.current() : 0;
					   const auto starsPerMessage =
						   (charging && charging->toggled()) ? state->starsPerMessage.current() : 0;
					   done({
						   restrictions,
						   slowmodeSeconds,
						   boostsUnrestrict,
...
Fn<void()> AboutGigagroupCallback(not_null<ChannelData *> channel, not_null<Window::SessionController *> controller) {
	...
	const auto convertSure = [=]
	{
		...
		*converting = true;
		channel->session()
			.api()
			.request(MTPchannels_ConvertToGigagroup(channel->inputChannel()))
			.done(
		...
		strong->show(Box(
			[=](not_null<Ui::GenericBox *> box)
			{
				...
				box->addRow(object_ptr<Ui::FlatLabel>(
					box, tr::lng_gigagroup_warning() | rpl::map(tr::rich), st::infoAboutGigagroup));
		...
		strong->show(Box(
			[=](not_null<Ui::GenericBox *> box)
			{
				...
				const auto addFeature = [&](rpl::producer<QString> text)
				{
					...
					box->addRow(
						object_ptr<Ui::FlatLabel>(box, std::move(text) | rpl::map(prefix + _1), st::infoAboutGigagroup),
						style::margins(
							st::boxRowPadding.left(), st::boxLittleSkip, st::boxRowPadding.right(), st::boxLittleSkip));
				};
...
std::vector<RestrictionLabel> RestrictionLabels(Data::RestrictionsSetOptions options) {
...
std::vector<AdminRightLabel> AdminRightLabels(Data::AdminRightsSetOptions options) {
...
EditFlagsControl<ChatRestrictions> CreateEditRestrictions(QWidget *parent,
														  ChatRestrictions restrictions,
														  base::flat_map<ChatRestrictions, QString> disabledMessages,
														  Data::RestrictionsSetOptions options) {
	auto widget = object_ptr<Ui::VerticalLayout>(parent);
	auto result = CreateEditFlags(widget.data(),
	...
	result.value = [original = std::move(result.value)] { return NegateRestrictions(original()); };
	result.changes = std::move(result.changes) | rpl::map(NegateRestrictions);

	return result;
}

EditFlagsControl<ChatAdminRights> CreateEditAdminRights(QWidget *parent,
...
ChatAdminRights DisabledByDefaultRestrictions(not_null<PeerData *> peer) {
	...
	return Flag(0) |
		((restrictions & Restriction::PinMessages) ? Flag(0) : Flag::PinMessages)
		//
		...
		//	: Flag::InviteByLinkOrAdd)
		//
		| ((restrictions & Restriction::ChangeInfo) ? Flag(0) : Flag::ChangeInfo);
}
...
ChatAdminRights AdminRightsForOwnershipTransfer(Data::AdminRightsSetOptions options) {
...
EditFlagsControl<PowerSaving::Flags> CreateEditPowerSaving(QWidget *parent,
	...
	auto result = CreateEditFlags(widget.data(), flags, std::move(descriptor));
	result.widget = std::move(widget);
...
CreateEditAdminLogFilter(QWidget *parent, AdminLog::FilterValue::Flags flags, bool isChannel) {
	auto widget = object_ptr<Ui::VerticalLayout>(parent);
...
EditFlagsControl<Data::ChatbotsPermissions> CreateEditChatbotPermissions(QWidget *parent,
																		 Data::ChatbotsPermissions flags) {
	auto widget = object_ptr<Ui::VerticalLayout>(parent);
	auto descriptor = Data::ChatbotsPermissionsLabels();
	descriptor.disabledMessages.emplace(Data::ChatbotsPermission::ViewMessages, QString());
	auto result = CreateEditFlags(widget.data(), flags | Data::ChatbotsPermission::ViewMessages, std::move(descriptor));
	result.widget = std::move(widget);
