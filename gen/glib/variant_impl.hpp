...
GLib::Variant base::VariantBase::new_from_data (const GLib::VariantType_Ref type, const guint8 * data, gsize size, gboolean trusted, GLib::DestroyNotify notify) noexcept
{
  typedef ::GVariant* (*call_wrap_t) (const ::GVariantType* type, const guint8* data, gsize size, gboolean trusted, GLib::DestroyNotify::cfunction_type notify, void* user_data);
  call_wrap_t call_wrap_v = (call_wrap_t) g_variant_new_from_data;
  auto notify_wrap_ = notify ? unwrap (std::move (notify), gi::scope_async) : nullptr;
  auto trusted_to_c = trusted;
  auto data_to_c = data;
  auto type_to_c = gi::unwrap (type, gi::transfer_none);
  auto _temp_ret = call_wrap_v ((const ::GVariantType*) (type_to_c), (const guint8*) (data_to_c), (gsize) (size), (gboolean) (trusted_to_c), (GLib::DestroyNotify::cfunction_type) (notify_wrap_ ? &notify_wrap_->wrapper : nullptr), (void*) (notify_wrap_));
  return gi::wrap (_temp_ret, gi::transfer_none);
}
