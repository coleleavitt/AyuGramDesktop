...
void VfsClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.local_file_add_info && factory) methods->local_file_add_info = (decltype (methods->local_file_add_info)) gi::detail::method_wrapper<self, void (*) (const char* filename, guint64 device, ::GFileAttributeMatcher* attribute_matcher, ::GFileInfo* info, ::GCancellable* cancellable, gpointer* extra_data, GDestroyNotify* free_extra_data), std::nullptr_t>::wrapper<&self::local_file_add_info_>;
...
void VfsClass::local_file_add_info_ (const char* filename, guint64 device, ::GFileAttributeMatcher* attribute_matcher, ::GFileInfo* info, ::GCancellable* cancellable, gpointer* extra_data, GDestroyNotify* free_extra_data) noexcept
{
  if (!get_struct_()->local_file_add_info) { g_critical ("no method in class struct"); return ; }
  typedef void (*call_wrap_t) (::GVfs*, const char*, guint64, ::GFileAttributeMatcher*, ::GFileInfo*, ::GCancellable*, gpointer*, GDestroyNotify*);
  call_wrap_t call_wrap_v = (call_wrap_t) get_struct_()->local_file_add_info;
  call_wrap_v (gobj_(), filename, device, attribute_matcher, info, cancellable, extra_data, free_extra_data);
}
