...
gi::result<Gio::Icon> base::IconBase::new_for_string (const gi::cstring_v str) noexcept
{
...
void IconIfaceDef::interface_init (gpointer interface_struct, gpointer factory)
{
  ...
  if (init_data.to_tokens && factory) methods->to_tokens = (decltype (methods->to_tokens)) gi::detail::method_wrapper<self, gboolean (*) (GPtrArray* tokens, gint* out_version), std::nullptr_t>::wrapper<&self::to_tokens_>;
}
...
gboolean IconIfaceClassImpl::to_tokens_ (GPtrArray* tokens, gint* out_version) noexcept
{
  if (!get_struct_()->to_tokens) { g_critical ("no method in class struct"); return {}; }
  typedef gboolean (*call_wrap_t) (::GIcon*, GPtrArray*, gint*);
  call_wrap_t call_wrap_v = (call_wrap_t) get_struct_()->to_tokens;
  auto result_ = call_wrap_v (gobj_(), tokens, out_version);
  return result_;
}
