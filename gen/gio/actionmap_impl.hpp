...
void ActionMapInterfaceDef::interface_init (gpointer interface_struct, gpointer factory)
{
  ...
  if (init_data.add_action) methods->add_action = (decltype (methods->add_action)) gi::detail::method_wrapper<self, void (*) (Gio::Action action), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::add_action_>;
  if (init_data.lookup_action) methods->lookup_action = (decltype (methods->lookup_action)) gi::detail::method_wrapper<self, Gio::Action (*) (const gi::cstring_v action_name), gi::transfer_none_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::lookup_action_>;
  if (init_data.remove_action) methods->remove_action = (decltype (methods->remove_action)) gi::detail::method_wrapper<self, void (*) (const gi::cstring_v action_name), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::remove_action_>;
}
