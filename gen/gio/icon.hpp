...
static GI_INLINE_DECL gi::result<Gio::Icon> new_for_string (const gi::cstring_v str) noexcept;
...
using GI_MEMBER_CHECK_CONFLICT(to_tokens) = self;
...
virtual gboolean to_tokens_ (GPtrArray* tokens, gint* out_version) noexcept = 0;
...
// gboolean Icon::to_tokens (::GIcon* icon /*none*/,  tokens /*none,out,ca*/, gint* out_version);
// SKIP; tokens type  not supported
GI_INLINE_DECL gboolean to_tokens_ (GPtrArray* tokens, gint* out_version) noexcept override;
...
struct IconIfaceDef::TypeInitData
{
  ...
  GI_MEMBER_DEFINE(IconIfaceClassImpl, to_tokens)
...
constexpr static TypeInitData factory()
{
  ...
  return {
    ...
    GI_MEMBER_HAS_DEFINITION(SubClass, DefData, to_tokens)
  };
}
};
} // namespace internal
