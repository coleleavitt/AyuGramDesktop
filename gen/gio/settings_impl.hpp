...
void SettingsClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.change_event && factory) methods->change_event = (decltype (methods->change_event)) gi::detail::method_wrapper<self, gboolean (*) (const GQuark* keys, gint n_keys), std::nullptr_t>::wrapper<&self::change_event_>;
...
gboolean SettingsClass::change_event_ (const GQuark* keys, gint n_keys) noexcept
{
  if (!get_struct_()->change_event) { g_critical ("no method in class struct"); return {}; }
  typedef gboolean (*call_wrap_t) (::GSettings*, const GQuark*, gint);
  call_wrap_t call_wrap_v = (call_wrap_t) get_struct_()->change_event;
  auto result_ = call_wrap_v (gobj_(), keys, n_keys);
  return result_;
}
