...
void SocketControlMessageClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.get_level) methods->get_level = (decltype (methods->get_level)) gi::detail::method_wrapper<self, gint (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_level_>;
  if (init_data.get_size) methods->get_size = (decltype (methods->get_size)) gi::detail::method_wrapper<self, gsize (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_size_>;
  if (init_data.get_type) methods->get_type = (decltype (methods->get_type)) gi::detail::method_wrapper<self, gint (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_type_>;
  if (init_data.serialize) methods->serialize = (decltype (methods->serialize)) gi::detail::method_wrapper<self, void (*) (void* data), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::serialize_>;
}
