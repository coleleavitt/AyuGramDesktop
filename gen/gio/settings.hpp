...
using GI_MEMBER_CHECK_CONFLICT(change_event) = self;
...
virtual gboolean change_event_ (const GQuark* keys, gint n_keys) noexcept = 0;
...
// gboolean Settings::change_event (::GSettings* settings /*none*/, const ::GQuark keys, gint n_keys);
// SKIP; inconsistent keys in pointer depth (1 vs 0)
GI_INLINE_DECL gboolean change_event_ (const GQuark* keys, gint n_keys) noexcept override;
...
{
  GI_MEMBER_DEFINE(SettingsClass, change_event)
...
constexpr static TypeInitData factory()
{
  ...
  return {
    GI_MEMBER_HAS_DEFINITION(SubClass, DefData, change_event),
