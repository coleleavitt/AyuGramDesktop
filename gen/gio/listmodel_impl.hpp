...
void ListModelInterfaceDef::interface_init (gpointer interface_struct, gpointer factory)
{
  ...
  if (init_data.get_item) methods->get_item = (decltype (methods->get_item)) gi::detail::method_wrapper<self, GObject::Object (*) (guint position), gi::transfer_full_t, std::tuple<gi::transfer_none_t>>::wrapper<&self::get_item_>;
  if (init_data.get_item_type) methods->get_item_type = (decltype (methods->get_item_type)) gi::detail::method_wrapper<self, GType (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_item_type_>;
  if (init_data.get_n_items) methods->get_n_items = (decltype (methods->get_n_items)) gi::detail::method_wrapper<self, guint (*) (), gi::transfer_none_t, std::tuple<>>::wrapper<&self::get_n_items_>;
}
