...
void TlsPasswordClassDef::class_init (gpointer class_struct, gpointer factory)
{
  ...
  if (init_data.get_value && factory) methods->get_value = (decltype (methods->get_value)) gi::detail::method_wrapper<self, const guchar* (*) (gsize* length), std::nullptr_t>::wrapper<&self::get_value_>;
  if (init_data.set_value && factory) methods->set_value = (decltype (methods->set_value)) gi::detail::method_wrapper<self, void (*) (guchar* value, gssize length, GDestroyNotify destroy), std::nullptr_t>::wrapper<&self::set_value_>;
}
...
const guchar* TlsPasswordClass::get_value_ (gsize* length) noexcept
{
  if (!get_struct_()->get_value) { g_critical ("no method in class struct"); return {}; }
  typedef const guchar* (*call_wrap_t) (::GTlsPassword*, gsize*);
  call_wrap_t call_wrap_v = (call_wrap_t) get_struct_()->get_value;
  auto result_ = call_wrap_v (gobj_(), length);
  return result_;
}
...
// SKIP; callback misses closure info
void TlsPasswordClass::set_value_ (guchar* value, gssize length, GDestroyNotify destroy) noexcept
{
  if (!get_struct_()->set_value) { g_critical ("no method in class struct"); return ; }
  typedef void (*call_wrap_t) (::GTlsPassword*, guchar*, gssize, GDestroyNotify);
  call_wrap_t call_wrap_v = (call_wrap_t) get_struct_()->set_value;
  call_wrap_v (gobj_(), value, length, destroy);
}
