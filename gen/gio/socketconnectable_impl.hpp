...
void SocketConnectableIfaceDef::interface_init (gpointer interface_struct, gpointer factory)
{
  ...
  if (init_data.enumerate) methods->enumerate = (decltype (methods->enumerate)) gi::detail::method_wrapper<self, Gio::SocketAddressEnumerator (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::enumerate_>;
  if (init_data.proxy_enumerate) methods->proxy_enumerate = (decltype (methods->proxy_enumerate)) gi::detail::method_wrapper<self, Gio::SocketAddressEnumerator (*) (), gi::transfer_full_t, std::tuple<>>::wrapper<&self::proxy_enumerate_>;
